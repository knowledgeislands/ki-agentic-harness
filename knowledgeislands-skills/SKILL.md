...
2. **Re-fetch each source** (WebFetch/WebSearch) and **diff against the current [rubric](references/rubric.md)**: new required/optional frontmatter fields,
   changed caps (length, line, token budgets), new anti-patterns, deprecations. Note where sources disagree.
3. **Scan our own skills** in the arcadia-skills repo for emergent patterns that work but aren't yet codified — promote the good ones into the rubric; flag
   drift that contradicts it.
4. **Propose a diff** to [the rubric](references/rubric.md) and, where relevant, [the linter](scripts/lint-skills.ts) (a newly-mechanical check should move from
   judgment into the script). Confirm before writing.
