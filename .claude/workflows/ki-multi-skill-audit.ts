...
1. COLL-1 (name uniqueness): Run \`bun run ki:skills:audit\` from the harness root. Capture the output verbatim. Report any FAIL or WARN findings about duplicate names.
...
3. Per-concern findings, ordered by dependency order (authoring → engineering → repo → kb → streams → mcp → website → website-cloudflare → agents → skills → tokenomics → harness). For each concern: FAIL items first, then WARN, then POLISH, then ADVISORY. Skip concerns with zero findings.
