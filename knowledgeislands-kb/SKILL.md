...
Almost everything is fixed by the structure above. Only these come from the host project - take them from the auto-loaded `CLAUDE.md`, then the root memory index, then the project's extension skill if it ships one:
...
3. Pre-flight before writing anything substantive: scope cascade loaded; if the work engages a named person/entity with a profile note, read it first; run any domain pre-flight the host `CLAUDE.md` or extension mandates.
...
- Where a base ships a named extension skill (one named for the base, e.g. `<base>-kb`), that skill takes precedence and supplies the base-specific pre-flight and scope rules; it delegates these five modes back here.
