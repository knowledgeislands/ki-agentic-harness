...
A Knowledge Islands base is a single markdown store organised into fixed zones - `+/` (inbound), `Calendar/`, `Pillars/`, `Resources/`, `Streams/`, `-/`
(outbound), and `Admin/`. The zone set is part of the standard, so the skill does not ask a base to define it; it only needs a few store-level bindings.
...
- **Island vs Pillar.** Each whole knowledge base is an "island" (a legal base, a personal base, a research base). Within a base, a **Pillar** is a major strand
  of subject matter - a case, a client, a domain, a theme. `Pillars/` replaces what some bases historically called `Matters/`.
