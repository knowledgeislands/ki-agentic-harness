...
## Layer 1 — local files (from a checkout; skipped in `--org` mode)
...
- **visibility [M]** visibility matches the name prefix (`arcadia-*` private, `mcp-*` public); unknown prefix → WARN. (standard: Visibility)
- **topics [M]** _(public)_ carries the standard topic set. (Layer 2)
- **protection [M]** _(public)_ `main` requires a PR, the `build` check, and linear history. (Layer 2)
...
- **exceptions [J]** a divergence the script flags is a genuine drift, not a documented **intentional exception** (e.g. private repos lacking branch protection
  / secret scanning). Don't re-flag exceptions. (standard: Intentional exceptions)
