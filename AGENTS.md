...
- **`docs/guides/user-guide/skills.md` embeds the generated `ki:skills:graph --tree` block** — `bun run test` compares its marked region with the live graph and fails on drift. When you add/remove a skill or change an `implies:` edge, regenerate it with `bun run ki:skills:graph` and replace the whole marked block; don't hand-edit just your node.
