...
- **Re-vendoring `.ki-meta/`** — package scripts run vendored checker copies, not `skills/*/scripts` source. After changing a coverage-scoped checker, format it then run `bun skills/keystone/ki-bootstrap/scripts/internal/repo-bootstrap.ts .` (never `--all`) before testing. `ki:bootstrap:audit` enforces source/vendor parity; expected manifest and `ki-educate` stamp drift follows each commit. The [onboarding guide](docs/guides/user-guide/onboarding.md) covers the model and recovery details.
