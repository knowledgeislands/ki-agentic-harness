...
[^ap]:
    ...
    geography) and its local specifics; its `Model/Conventions/Structure` Streams section likewise points here. Conformed to the canonical
    model on 2026-06-04 (the `Proposal` suffix, `type:` scheme, machine-readable proposal frontmatter, `Governance` footers).
