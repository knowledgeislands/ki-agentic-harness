...
- Skill-id lag: `arcadia-principal`'s CLAUDE.md and process note reference `knowledgeislands-streams`; the harness skill is now `ki-kb-streams`. Tracked centrally under the `ki-kb-*` rename, not a model change.
