...
Keep every table **skimmable at a glance**. Prettier pads columns to align them but never breaks a cell across lines, so a cell with too
much content forces a very wide row that's unreadable in a terminal or in print. Aim to keep each row within a comfortable reading width (≈
100 characters is a good target for content meant to be read in a terminal or printed).

When a column's content would force the table to wrap awkwardly or demand cryptic abbreviations in the headers, **move the long content into
footnotes below the table** and leave a footnote marker in the cell instead.

**Prefer footnotes over** shrinking headers to cryptic abbreviations, wrapping mid-cell, or dropping a useful column. The table stays
scannable; the detail sits just below it.
