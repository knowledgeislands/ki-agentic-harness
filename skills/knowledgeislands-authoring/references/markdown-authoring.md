...
- **Refer to another skill by its `name`**, never by a file path — "the `knowledgeislands-kb` skill" — because a skill's location on disk is not stable, but its name is how it loads into the session.
