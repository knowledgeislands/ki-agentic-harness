...
3. **Compose on `knowledgeislands-kb`** — zone and zone-index checks are owned by `knowledgeislands-kb`; run its audit for the base first (the `Admin/Operations/` zone) and note its result rather than re-deriving it here.
