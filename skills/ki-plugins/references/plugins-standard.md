...
`ki-binding` owns **generation** (`build-plugin.ts`) and **cross-surface enablement** (BIND-4 checks the machine's `cowork_settings.json` points at the repo and enables the plugin). `ki-plugins` owns the **repo's on-disk shape**. The two do not re-check each other's half. This is composition — each skill runs its own lane and declares the edge (`ADR-KI-HARNESS-005`).
