 ...
 * audit-plugins.ts's findings that are unambiguous and reversible, leaving
 ...
 * house conform shape (conform-repo.ts, conform-drs.ts) —
 * `bun scripts/conform-plugins.ts .` / `ki:plugins:conform`. The marketplace
 * manifest path, the org constant, and the plugin-entry shape are kept in
 * lockstep with audit-plugins.ts (same source of truth, copied rather than
 * imported so each script stays valid standalone per the composition-only rule).
 *
 *   bun scripts/conform-plugins.ts [path]   # default: cwd
 *   --dry-run                                 # print the plan, mutate nothing
 *
 * Fixes:
 *   - marketplace.json `owner.name` → the known-good org string audit-plugins.ts
...
// ── kept in lockstep with audit-plugins.ts ──
...
function harnessPackageVersion(): string | null {
  // This script lives at <harness>/skills/ki-plugins/scripts/conform-plugins.ts —
...
async function main() {
  ...
  console.log(
    '  - Everything else audit-plugins.ts flags (repo scaffold files, [ki-plugins] .ki-config.toml marker, plugin name/source-dir agreement, stale projection) is authoring/regeneration judgment.'
  )
  console.log(
    `\n${paint(C.dim, 'mechanical layer applied — re-run `bun skills/ki-plugins/scripts/audit-plugins.ts` (or `ki:plugins:audit`) to confirm findings clear.')}`
  )
}
