...
INIT scaffolds no standalone artifact — the single source (`mcps.yaml`) lives in chezmoi, never in a target repo. It vendors this skill's declared mechanical unit (the frontmatter `vendors:` declaration — the checker and the Cowork conformer) into the target's `.ki-meta/` via the central bootstrap chain: [`scripts/init.ts`](scripts/init.ts) is a thin delegator that execs the `ki-bootstrap` engine with this skill as an explicit seed.
