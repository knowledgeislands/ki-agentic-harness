...
- [AH] the previous note recorded **thirteen** skills with the eval suite covering twelve of thirteen. The harness now holds **18** skills under `skills/` (`ki-11ty-websites`, `ki-activities`, `ki-agents`, `ki-authoring`, `ki-bootstrap`, `ki-cloudflare-hosting`, `ki-decision-records`, `ki-engineering`, `ki-handoffs`, `ki-harness`, `ki-kb`, `ki-live-artifacts`, `ki-mcp`, `ki-plans`, `ki-repo`, `ki-skills`, `ki-streams`, `ki-tokenomics`). The `agents/` shelf is populated (governance agents); `mcp/` and `evals/` remain valid shelves. The `harness-standard.md` body is deliberately **count-agnostic** and needs no edit for this growth — the container it describes still matches current reality.
