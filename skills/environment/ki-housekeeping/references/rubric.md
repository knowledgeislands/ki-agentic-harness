...
Used by Mode AUDIT. These are the criteria for the **memory area**, the one area governed locally in full (see [standards.md](standards.md) §2). The **other areas** (sessions, artifacts / outputs, backups, plugins, project cache) are audited through the paired `mcp-claude-housekeeping` server's codified per-surface audits, not by these criteria; the skill applies judgment over the server's findings.
...
- **[M] SELF-1** For every recognised runtime declared by `[ki-repo].supported_runtimes`, the repository contains the `ki-self` payload at that runtime's discovery path: `.claude/skills/ki-self/SKILL.md` for `claude-code`; `.agents/skills/ki-self/SKILL.md` for `codex`. Missing is a WARN. A symlink or non-regular file is a FAIL.
