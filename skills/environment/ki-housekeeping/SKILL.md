...
| EDUCATE | Scaffolds no standalone artifact — the state this skill governs already accumulates outside the repo tree. It vendors the skill's `scripts/govern.ts` entrypoint into the target's `.ki/bootstrap/` via the central bootstrap chain: [`scripts/educate.ts`](scripts/educate.ts) is a thin delegator into the `ki-bootstrap` engine. |
