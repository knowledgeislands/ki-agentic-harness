 ...
 *
 * Covers IDX-6 (headroom:learn entries rooted in another repo) in the canonical
 * checker-reporter JSONL stream: a foreign-repo capture must WARN and cite IDX-6;
 * a clean own-repo block must stay silent.
 */
...
// ── Foreign: a learn-block line rooted in another KI repo → IDX-6 WARN ──
...
try {
  ...
  check('foreign root → WARN severity', /"level":"WARN","code":"IDX-6"/.test(out))
...
// ── Control: a learn-block referencing only this repo → no IDX-6 ──
const clean = fixture(
...
try {
  ...
  check('matching local runtime payloads → SELF-1 PASS', /"level":"PASS","code":"SELF-1"/.test(out))
...
try {
  ...
  check(
    'missing declared Codex payload → SELF-1 WARN',
    /"level":"WARN","code":"SELF-1"/.test(out) && out.includes('.agents/skills/ki-self/SKILL.md')
  )
} finally {
...
try {
  ...
  check('symlinked declared payload → SELF-1 FAIL', /"level":"FAIL","code":"SELF-1"/.test(out) && out.includes('owned regular file'))
} finally {
