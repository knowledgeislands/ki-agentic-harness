const DOC_ITEMS = [
  ...
  ['DOC-2', '`project` memories use absolute dates, not relative ones ("2026-03-05", not "Thursday").'],
  [
    'DOC-3',
  ...
  ['DOC-6', '`MEMORY.md` entries are organized semantically by topic, not chronologically.']
].map(([code, description]) => ({
  code,
  title: code === 'DOC-6' ? 'Semantic index ordering' : 'Content doctrine',
  description,
  sources: ['rubric.md'],
