#!/usr/bin/env bun
/**
 * Run-based behavioural test for `audit-memory.ts`.
 *
...
const CHECKER = join(dirname(fileURLToPath(import.meta.url)), 'audit-memory.ts')
...
const clean = fixture('- Command `bun skills/ki-mcp/scripts/audit-mcp.ts ../mcp-gsuite` from knowledgeislands/ki-agentic-harness works.')
try {
...
if (failed) {
  console.log('\n\x1b[31maudit-memory.test.ts: failures\x1b[0m')
  process.exit(1)
}
console.log('\n\x1b[32maudit-memory.test.ts: all checks passed\x1b[0m')
