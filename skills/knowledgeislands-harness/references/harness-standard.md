...
| --------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------- |
| `skills/` | [Agent Skills](https://agentskills.io/specification) — one directory per skill | Symlinked to `~/.claude/skills/` via `bun run skills:link`  |
...
the skill loads under the wrong name or fails to load at all. The `skills:link` script (see §package.json) creates symlinks named by the
directory, so the symlink target name is the one the agent actually resolves.
...
4. **Toolchain** — the key `bun run *` commands: at minimum `skills:link`, `skills:lint`, and the common `lint:*` family. Enough to orient a
...
| --------------- | -------------------------------------------------------------------- | ----------------------------- |
| `skills:link`   | Symlinks every `skills/<name>` into `~/.claude/skills/`              | The primary install mechanism |
...
The harness-specific scripts are `skills:link` and `skills:lint` — these are the delivery and quality mechanisms the harness concept depends
on. Absence of either is a FAIL.
