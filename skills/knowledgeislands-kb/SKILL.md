...
description: >
  ...
  digest", "audit my knowledge base", "is my base structured right", "set up a new knowledge base". For the `Streams` zone (proposals, the
  Enactment Process) use the `knowledgeislands-streams` skill it delegates to; for general Markdown or TOML house style (not note content)
  use `knowledgeislands-authoring`.
