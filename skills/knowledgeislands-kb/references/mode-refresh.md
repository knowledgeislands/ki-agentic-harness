...
_On-demand procedure for kb's REFRESH mode. The shared model — the five-zone structure, routing test, memory cascade, project bindings, and
...
1. **Read [the source list](sources.md)** - the canonical structure definition (arcadia-agentic-harness `docs/knowledge-islands.md`) and the
...
5. **Update [the source list](sources.md)** - bump each `last reviewed` date, add any new source, retire any dead one. The record of what
   changed is the commit itself - history lives in git, not a changelog. This step is mandatory: the source list is the skill's memory of
   where its structure model comes from.
