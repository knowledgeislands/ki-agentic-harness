...
_On-demand procedure for kb's AUDIT and CONFORM modes (CONFORM runs AUDIT first, so they share this file). The shared model — the five-zone
structure, routing test, memory cascade, project bindings, and Step 1 (Load context) — lives in [`SKILL.md`](../SKILL.md) and is already
loaded; this file is the procedure only._
...
2. **Apply the judgment layer by reading** - the **[J]** criteria in [the rubric](audit-rubric.md) that the script cannot judge: notes filed
