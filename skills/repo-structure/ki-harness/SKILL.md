...
- The `ki:skills:copy:project` install convention is the harness's primary project delivery mechanism — verifying it is wired in `package.json` is a FAIL criterion, not advisory. `ki:repo:link-commands` composes the explicitly opt-in local-development path.
