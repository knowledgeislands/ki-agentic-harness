...
Severity: **FAIL** (ship-stopper — the site won't build or `dist/` isn't portable), **WARN** (layout / config / Tailwind divergence), **POLISH** (SEO / consistency) — the shared ladder, defined in `ki-engineering`'s [`enforcement-framework.md`](../../../general-governance/ki-skills/references/enforcement-framework.md) §2.
