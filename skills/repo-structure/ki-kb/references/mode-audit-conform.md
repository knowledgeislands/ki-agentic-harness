...
1. **Run the mechanical checker** - `bun scripts/audit.ts <base-path>` (from this skill's directory). It reports the deterministic layer as canonical checker-reporter JSONL (FAIL / WARN / POLISH / ADVISORY / INFO / NA / PASS — see `ki-engineering`'s enforcement-framework §2) and exits non-zero on any mechanical FAIL: the five zones present (resolved through any `[ki-kb.zones]` alias), a same-name index note per zone, the root `Admin/MEMORY.md`, the base's own `[ki-kb]` table validated _down_, and note frontmatter (well-formed `---` fences, snake_case keys, and any keys declared in `required_frontmatter`). The aggregate handles presentation. Capture its output; do not re-derive what it checks.
2. **Apply the judgment layer by reading** - the **[J]** criteria in [the rubric](rubric.md) that the script cannot judge: notes filed in the wrong zone (per the routing test), _whether a note should carry frontmatter at all_ and its naming quality, whether the memory index's active-Pillar list is actually accurate, and fact-vs-analysis labelling where the base distinguishes them.
