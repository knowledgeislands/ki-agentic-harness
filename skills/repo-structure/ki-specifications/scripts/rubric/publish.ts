...
const publication = fileURLToPath(new URL('../../references/rubric.md', import.meta.url))
const renderItem = (item: RubricItem<never>): string => `- **${item.code} [${rubricTypes(item).map((type) => type === 'MECHANICAL' ? 'M' : 'J').join(' + ')}] — ${item.title}** — ${item.description} (${item.sources.join(', ')})${item.judgment ? `\n  - _Review prompt:_ ${item.judgment.prompt}` : ''}`
...
export const publishRubric = (): void => writeFileSync(publication, renderRubric(KI_SPECIFICATIONS_RUBRIC))
