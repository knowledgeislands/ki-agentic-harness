...
export const createSpecificationsContext = ({ target, dryRun }: { target: string; dryRun: boolean }): SpecificationsContext => {
  ...
  return {
    targetExists, configExists, malformed, table,
    applicable: Boolean(table || malformed || coreEvidence.some((entry) => entry.exists)),
    core: coreEvidence, supporting: supportingEvidence,
    conformMarker: () => {
      if (!targetExists) return [{ status: 'VIOLATION', level: 'FAIL', message: 'Conform target must be an existing directory.' }]
      if (!configExists) return [{ status: 'VIOLATION', level: 'FAIL', message: '.ki-config.toml is absent; establish the repository with ki-repo first.', subject: '.ki-config.toml' }]
      const current = readFileSync(configPath, 'utf8')
      let parsed: Record<string, unknown>
      try { parsed = Bun.TOML.parse(current) as Record<string, unknown> } catch { return [{ status: 'VIOLATION', level: 'FAIL', message: '.ki-config.toml is malformed; repair it before conforming.', subject: '.ki-config.toml' }] }
      if (parsed[SECTION] !== undefined) return [{ status: 'PASS', message: 'The [ki-specifications] marker is already present.', subject: '.ki-config.toml' }]
      if (!dryRun) writeFileSync(configPath, `${current.trimEnd()}\n\n# This repo carries the KI Specifications repository structure.\n[${SECTION}]\n`)
      return [{ status: 'FIXED', message: `The [ki-specifications] marker ${dryRun ? 'would be added' : 'was added'}.`, subject: '.ki-config.toml' }]
    }
  }
}
