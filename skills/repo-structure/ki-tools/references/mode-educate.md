...
4. **Declare the config tables.** `bun scripts/govern.ts --educate >> .ki-config.toml` to add the `[ki-tools]` marker (alongside `[ki-repo]`). If the tool has a `package.json`, also declare `[ki-engineering]` and run its EDUCATE.
