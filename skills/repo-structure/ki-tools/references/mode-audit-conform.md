...
2. **Run the mechanical checker.** `bun scripts/audit.ts <repo-path>` covers the container: `bin/` + exec bit (FAIL), `install.sh`, `--version`, `CHANGELOG.md`, CI workflow, `tests/`, the shell capability conditionals (shellcheck + bats), the `package.json`→`ki-engineering` note, and the `[ki-tools]` marker. It grades on the unified severity ladder, exits non-zero on any mechanical FAIL, and emits canonical JSON Lines to stdout. Capture that stream verbatim — downstream tooling renders it; do not re-derive what it found.
