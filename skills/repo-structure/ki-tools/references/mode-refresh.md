...
2. **Re-fetch each source** (WebFetch/WebSearch) and **diff against the [standard](standards.md) + [rubric](rubric.md) + [`scripts/audit.ts`](../scripts/audit.ts)**. Look for: changed shellcheck defaults/severities, bats-core API or install changes, a new keep-a-changelog / semver revision, XDG spec changes, and any new capability signal the reference repo has adopted.
