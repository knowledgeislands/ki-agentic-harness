#!/usr/bin/env bun
/** The single governed entrypoint for ki-tools. */
...
Govern ki-tools through one standard entrypoint.
`
...
const checker = defineStructuredGovernedChecker({
  ...
  familyCodes: (_mode, options) => {
    const context = createToolsContext({ target: options.target, dryRun: options.dryRun })
    return context.targetExists && !context.applicable ? ['CONFIG'] : KI_TOOLS_FAMILY_CODES
  },
  context: (_mode, options) => () => createToolsContext({ target: options.target, dryRun: options.dryRun })
})
  ...
  runGovernedCli(
    {
      ...
      educate: (arguments_) => runSkillEducator({ skill: 'ki-tools', source: resolve(import.meta.dirname, '..'), argv: arguments_ })
    },
    argv
  )
