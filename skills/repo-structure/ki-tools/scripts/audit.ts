...
if (argv.includes('-h') || argv.includes('--help')) {
  process.stdout.write(
    'Usage: bun scripts/audit.ts [target] [--educate] [--reporter jsonl|terminal]\n\nAudit a Knowledge Islands tools repository.\n'
  )
  process.exit(0)
}
if (argv.includes('--educate')) {
  process.stdout.write('# ki-tools — opt-in marker for the tools repository standard\n[ki-tools]\n')
  process.exit(0)
}
...
const context = createToolsContext({ target, dryRun: true })
const familyCodes = context.targetExists && !context.applicable ? (['CONFIG'] as const) : KI_TOOLS_FAMILY_CODES
const result = runChecker({
  mode: 'audit',
  concern: KI_TOOLS_RUBRIC.concern,
  target,
  rubric: KI_TOOLS_RUBRIC,
  subjects: [{ familyCodes, context: () => context }]
})
