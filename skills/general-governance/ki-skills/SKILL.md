...
checker-modules: [rubric, checker]
description: >
...
1. **Run the linter.** `bun scripts/audit.ts <path-to-skill-or-repo>` from this skill's directory (or `bun run ki:skills:audit` at the ki-agentic-harness repo root). It returns the complete [canonical checker response](references/checker-response.md) using `FAIL`, `WARN`, `INFO`, `NOT_APPLICABLE`, and `PASS`, and exits non-zero on any `FAIL` finding. Judgment aspects are not emitted as synthetic findings; the response summary counts them as unevaluated. Capture the response verbatim — do not re-derive what it found. Point it at the **repo**, not a lone skill, so the cross-skill collision pass (COLL-1) has the siblings to compare.
