...
The conventions a good skill follows — what each is and why — live in [the Agent Skills standard](references/standards.md); the line-by-line checkable criteria (with `[M]`/`[J]` tags and codes) live in [the rubric](references/rubric.md), each citing its standard section. The canonical machine result emitted by governance checkers is defined in [the checker-report contract](references/checker-report.md) and its executable [schema](assets/checker-report.schema.json). Load the standard and rubric before an AUDIT, CONFORM, or EDUCATE; additionally load the checker-report contract when writing or reviewing a checker.
...
1. **Run the linter.** `bun scripts/audit.ts <path-to-skill-or-repo>` from this skill's directory (or `bun run ki:skills:audit` at the ki-agentic-harness repo root). It reports the mechanical criteria on the unified severity ladder (FAIL / WARN / POLISH / ADVISORY / INFO / NA / PASS), emits the [canonical checker report](references/checker-report.md), and exits non-zero on any FAIL M finding. Capture its output verbatim — do not re-derive what it found. Point it at the **repo**, not a lone skill, so the cross-skill collision pass (COLL-1) has the siblings to compare.
...
- Checker output conforms to the [canonical checker report](references/checker-report.md); `ki-engineering` retains the shared severity ladder as a technical primitive.
