...
The conventions a good skill follows — what each is and why — live in [the Agent Skills standard](references/standards.md); the line-by-line checkable criteria (with `[M]`/`[J]` tags and codes) live in [the rubric](references/rubric.md), each citing its standard section. The canonical machine result emitted by governance checkers is defined in [the canonical checker reporter](references/checker-reporter.md) and its executable [schema](assets/checker-reporter.schema.json). The executable contract for independently schedulable mode work is [the mode-elements schema](assets/mode-elements.schema.json), instantiated locally as `.ki-meta/mode-elements.json`. Load the standard and rubric before an AUDIT, CONFORM, or EDUCATE; additionally load the canonical checker reporter when writing or reviewing a checker.
...
- Checker output conforms to the [canonical checker reporter](references/checker-reporter.md); `ki-engineering` retains the shared severity ladder as a technical primitive.
