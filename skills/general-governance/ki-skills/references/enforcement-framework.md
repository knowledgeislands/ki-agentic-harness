...
- **One governance-mode model (SHAPE-5).** The universal EDUCATE/AUDIT/CONFORM/REFRESH plus skill-specific modes, so a new skill inherits the shape. EDUCATE's mechanical half is the per-skill `scripts/educate.ts` (§5).
