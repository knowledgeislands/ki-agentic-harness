...
A **governance skill** holds a house standard and ships the universal modes over it, backed by a tracked source list. `ki-skills` owns this framework because it is the self-governing root of the governance-skill system; every other governance skill in the set conforms to it.
...
- **EDUCATE** — scaffold a new artifact to the standard, or bring an off-standard one onto it from scratch. Its mechanical half is a per-skill **`scripts/educate.ts`** — the EDUCATE counterpart to AUDIT's `audit-*.ts` — that detects the existing shape, applies safe defaults, validates each selected skill's **`depends-on:`** requirements (the frontmatter graph, rendered by `ki:skills:graph`), and satisfies the self-sufficiency contract: it runs from the remote source with **no skill installed locally**, vendoring script copies (SCRIPT-7) and HELP snapshots into `.ki-meta/` and writing the four `.ki-meta/bin/{ki-audit,ki-conform,ki-educate,ki-help}` wrappers, so the target governs itself afterwards with zero skills installed; it does **not** write `package.json` (that is `ki-engineering`'s own concern, as sugar over the bins). Under an LLM it also applies judgment (choosing among shapes, filling prose); as pure CLI it takes the safe defaults and **prints the judgment checks it skipped**. `ki-bootstrap` starts the chain (always seeds `ki-repo`); re-running the idempotent chain is the single way to bring a target up to date.
...
- **One governance-mode model (KI-SHAPE-5).** The universal EDUCATE/AUDIT/CONFORM/REFRESH plus skill-specific modes, so a new skill inherits the shape. EDUCATE's mechanical half is the per-skill `scripts/educate.ts` (§5).
- **A behaviour-changing skill anchors its gate, and checks the anchor (KI-SHAPE-7).** A skill that changes a default cannot rely on its own `description` firing; it anchors the behaviour in always-loaded context (a repo/base `CLAUDE.md`/`AGENTS.md`) and its checker verifies the anchor.
- **Audits compose.** No single skill's pass means a target is clean; the applicable skills compose (§5).
- **Standard, not base-coupled extension (KI-SHAPE-2).** A standard skill stays base-agnostic and resolves bindings at runtime; what a base needs differently is **declared, not forked** — data in its `.ki-config` table, prose in its `CLAUDE.md` — never a `<base>-*` skill that takes the shared modes. Composition is the only inter-skill relationship (§5).
- **Mechanical work belongs in the checker, not in tokens (KI-SHAPE-9).** A criterion a script can decide with no AI benefit is tagged **[M]** and implemented in the checker; the reader's context is spent only on the **[J]** criteria that genuinely need judgment. A mechanical criterion left to prose is a finding, and the checker flags a rubric carrying **[M]** criteria but shipping no checker (§3).

(The criterion ids — LONG-1/2, COLL-1/2, KI-SHAPE-2/5/7/9 — are owned and enforced by the `ki-skills` rubric; named here so this framework and that rubric stay in lockstep.)
