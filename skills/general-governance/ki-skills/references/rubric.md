...
Checker output follows the canonical report and exit-code contract in [checker-report.md](checker-report.md); `ki-engineering` owns the shared severity ladder.
...
- **SCRIPT-9 [M]** A skill's `scripts/**/*.ts` files contain no static `from`, dynamic `import()`, or CommonJS `require()` relative import that resolves outside that skill's own `scripts/` directory. `ki-bootstrap` vendors a skill's mechanical unit as a standalone payload into every governed repo's `.ki-meta/skills/<skill>/` ([ADR-KI-HARNESS-006](../../../../docs/decisions/ADR-KI-HARNESS-006-bootstrapping-and-self-sufficiency.md)); no sibling skill directory or other source file is implicitly available. The payload may only import files packaged within its own `scripts/` directory. (KI)
- **ROOT-1 [M]** `ki-skills` is the self-governing checker-contract root: it declares `checker-supports: [checker-report]`, ships `scripts/checker-report.ts`, and declares no `checker-requires:` entry. Its report contract therefore runs from its own shipped files without a dependency on itself or another skill. Other skills may request only declared support modules, which bootstrap copies into their local `scripts/vendored/<provider>/` namespace; that request is implementation packaging, not `implies:` or composition. (ADR-KI-HARNESS-SKILLS-012)
...
- **SHAPE-8 [M + J]** _Governance-skill checker contract._ A governance skill's audit and conform scripts emit the [canonical checker report](checker-report.md) by default and may write that same report to `.ki-meta/audits/<concern>.json`. Exit code is non-zero iff an M finding is FAIL; WARN / POLISH / ADVISORY / INFO / NA / PASS all exit 0. Every declared `[J]` criterion appears once as a cited J/ADVISORY review prompt. Findings use the unified severity ladder defined in `ki-engineering`'s `enforcement-framework.md` §2. The linter mechanically **[M]** verifies each `audit-*.ts` / `lint-*.ts` source carries the standardised remediation route (`→ to address: run /<skill> CONFORM …`) naming its **own** skill (a missing route, or one naming another skill, WARNs); report shape, summary agreement, J coverage, and exit-code behaviour are verified by the source-harness checker-report test. (enforcement-framework.md §2/§5, checker-report.md)
