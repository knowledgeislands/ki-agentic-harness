...
- **ROOT-1 [M]** `ki-skills` is the self-governing checker-contract root: it declares `checker-modules: [checker-reporter]`, ships `scripts/checker-reporter.ts`, and declares no `checker-dependencies:` entry. Its canonical checker reporter therefore runs from its own shipped files without a dependency on itself or another skill. Other skills may declare only offered checker modules, which bootstrap copies into their local `scripts/vendored/<provider>/` namespace; that declaration is implementation packaging, not `implies:` or composition. (ADR-KI-HARNESS-SKILLS-012)
