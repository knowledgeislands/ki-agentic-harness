...
The module name is extension-free and identifies one local payload in the provider's `scripts/lib/`: either `scripts/lib/<module>.ts` or a directory `scripts/lib/<module>/` containing its own local closure.
