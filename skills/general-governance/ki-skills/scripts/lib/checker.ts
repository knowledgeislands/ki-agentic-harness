...
const executeRubric = <RootContext>(input: CheckerInput<RootContext>): CheckerFinding[] => {
  ...
  for (const planned of planExecutions(input)) {
    const rootContext = planned.subject.context()
...
export const validateCheckerRecords = <RootContext>({
  ...
  for (const [index, value] of records.entries()) {
    ...
    if (!level) errors.push(`${label} level is not recognised`)
      ...
      if (item?.mechanical && (level === 'FAIL' || level === 'WARN') && level !== item.mechanical.level)
        errors.push(`${label} violation level does not match rubric item: ${code}`)
