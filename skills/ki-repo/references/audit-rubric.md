...
Every **[M]** finding here is also auto-fixable: [`../scripts/conform.ts`](../scripts/conform.ts) applies the matching `gh` call or local scaffold directly (`--dry-run` to preview). The **[J]** findings (README content, description text/visibility, whether a `[ki-repo.checks]` override is warranted) are printed as manual TODOs, never guessed.
