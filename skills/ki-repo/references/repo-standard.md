...
**Self-check capability is required.** A confirmed ki-repo must carry a way to run its checks with zero skills installed: `.ki-meta/bin/ki-audit` or `.ki-meta/aggregate.ts` must be present. A marker-only repo with neither is a FAIL (`self-check`) — re-bootstrap to write the runner.
