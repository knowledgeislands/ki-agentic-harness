...
_On-demand procedure for streams' REFRESH mode. The shared model — the zone-at-a-glance, the status lifecycle, the proposal anatomy, the
...
5. **Update [the source list](sources.md)** — bump each `last reviewed` date. The record of what changed is the commit itself — history
   lives in git, not a changelog. Mandatory.
