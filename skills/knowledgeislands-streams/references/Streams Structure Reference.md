...
`Streams/` carries knowledge **in motion**: active projects, evolving ideas, ongoing work. Its content is _not_ canonical — durable knowledge lives in `Pillars/` (internal; a base that holds it under a legacy folder name resolves it via the `knowledgeislands-kb` zone alias) or `Resources/` (external). The lifecycle of a stream is: emerge → mature through work → stabilise into a store → the stream is retired. A stream is a **status tracker and proposal document**, never a knowledge store; a stream note that accumulates deep subject-matter content is a signal that the content needs to move out.
