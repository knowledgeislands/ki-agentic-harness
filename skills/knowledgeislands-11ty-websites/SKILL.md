...
When the repo also holds **unrelated deployables** (a bot, an ingress Worker — out of this skill's scope), they are added as sibling
workspaces (`["site", "ingress"]`). Each workspace owns its own `dist/`; `site/` always emits to `./dist` (`site/dist/`).
...
3. **TypeScript runs natively — no transpile.** `eleventy.config.ts` and `_data/*.ts` run under Bun; `.ts` + `.json5` data extensions are
   registered in the config. `tsc` is type-check only (engineering's layer).
...
`dist/` is gitignored and regenerated by the build. `dist/` lives inside the `site/` workspace (`site/dist/`) — the path
