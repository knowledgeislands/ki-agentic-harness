...
- **`return { dir: { input: 'src', output: './dist', includes: '_includes', data: '_data' }, htmlTemplateEngine: 'njk', markdownTemplateEngine: 'njk', templateFormats: ['njk','md','html'] }`**
  — output resolves to `dist/` inside the `site/` workspace (`site/dist/`), per §2.
...
`dist/` sits inside the `site/` workspace (`site/dist/`), per §2. The hosting skill points `assets.directory` at `./dist` from
`site/wrangler.jsonc`. **Building `dist/` is this skill; serving it is `knowledgeislands-cloudflare-hosting`.**
