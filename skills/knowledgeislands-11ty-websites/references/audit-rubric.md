...
[`../scripts/audit-websites.ts`](../scripts/audit-websites.ts) for the mechanical items (marked **[M]**), then judge the rest by reading.
Each item cites the standard section it verifies.
...
- [ ] [J] WARN — Nunjucks is the template engine (`htmlTemplateEngine`/`markdownTemplateEngine` = `'njk'`); content is `.md`, logic is
      `.njk`. (§1)
...
- [ ] [M] FAIL — a transform rewrites absolute internal URLs to relative (the portable-`dist/` transform; `toRelativeOutputUrl` /
...
- [ ] [J] WARN — `tokens.css` defines the semantic palette in `@layer base :root {}` (`--background`/`--foreground`/`--primary`/… +
      brand/layout vars), sampled from the site's imagery; self-hosted fonts use `@font-face` + `font-display: swap`. (§5)
...
- [ ] [J] FAIL — the build emits relative internal links (the §4 transform actually fires over `.html`), so `dist/` serves from any root.
      (§9)
- [ ] [J] WARN — `dist/` is never hand-edited; it is fully regenerated by the build. (§9)
- [ ] [J] POLISH — `assets.directory` in `site/wrangler.jsonc` is `"./dist"` (pointing at `site/dist/`) — verified by
      `knowledgeislands-cloudflare-hosting`, named here as the seam. (§9)

## Longevity & staleness (§1)

Mirrors the `knowledgeislands-skills` rubric's **LONG-1**.

- [ ] [J] WARN — volatile facts (Eleventy/Tailwind/Lucide versions, the spec idioms the config relies on) sit in `package.json` / the
      standard, not scattered — a bump is one known edit.
- [ ] [J] POLISH — this audit runs against a **current** standard: a cited requirement is confirmed by Mode REFRESH +
      [`sources.md`](sources.md) not having gone stale since its `last reviewed` date.
