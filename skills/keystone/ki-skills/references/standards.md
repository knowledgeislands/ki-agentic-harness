...
**Shared modules are the narrow implementation exception.** A provider exposes modules with `ki-shared-modules:`; a dependent names each exact `provider:module` reference with `ki-shared-dependencies:`. The extension-free module name resolves to one safe provider file at `scripts/shared/<module>.ts`. The active installed collection carries the complete verified module graph, so a native operation imports only the resolved collection path, never a sibling source path, a copied repository payload, or a checkout-relative path. The declaration creates no governance coverage or composition edge. `ki-skills` is the checker-contract root: it provides the canonical `rubric`, `checker`, and `reporter` modules from its own shipped files and uses those owned modules directly. Current `scripts/vendored/` copies are legacy migration inputs only, not a delivery contract. (ADR-KI-HARNESS-SKILLS-012, [installed skill collection contract](../../../../docs/decisions/references/installed-skill-collection-contract.md))
...
A **governance skill** — one that holds a house standard — exposes a common mode set so a reader moves between skills without relearning: the universal four are **AUDIT** (check an artifact against the standard), **CONFORM** (bring an existing artifact into line), **EDUCATE** (bring the skill's governance into being in a target: scaffold a new conformant artifact or bring an off-standard one onto the floor), and **REFRESH** (re-anchor the standard to its sources). Modes beyond the four are fixed in meaning where they appear: **NEW** authors one new instance into a collection the skill governs (present only in collection skills, presupposing EDUCATE, never a substitute for it), **OPTIMISE** pushes a compliant artifact toward excellent, and operational modes serve a skill's own domain (the `ki-kb` note-ops — DIGEST / EXTRACT / QUERY / SAVE / UPDATE). The modes live under a single `## Operating modes` H2, each as a `### Mode <NAME>` H3 or — for router skills — rows of a `| Mode | … |` dispatch table, with every `argument-hint` verb present in that section (rubric **KI-SHAPE-12**, **KI-SHAPE-13**). In the target delivery model, a governance skill registers manifest-covered in-process AUDIT and CONFORM operations in the verified collection; `ki` resolves and hosts them, and REFRESH remains harness-only. The present `scripts/govern.ts` and `scripts/educate.ts` shape is a source-authoring and migration implementation invariant, not a repository delivery contract. All `ki-*` skills are governance skills on this model. (ki-agentic-harness README, ADR-KI-HARNESS-SKILLS-001, ADR-KI-HARNESS-007, [installed skill collection contract](../../../../docs/decisions/references/installed-skill-collection-contract.md))

Within a **Knowledge Islands repo** (one carrying a `.ki-config.toml`), a governance skill also takes a shared **file shape** so a reader — or a new such skill — moves between them without relearning: its primary normative reference is `references/standards.md`; its pass/fail criteria are `references/rubric.md`; and its tracked provenance is `references/sources.md`. Use `references/exemplars.md` for optional worked examples and `references/mode-<verb>.md` for independently invoked mode procedures, co-locating tightly coupled modes. A skill with a genuinely separate secondary normative topic gives only that file a descriptive `<topic>-standards.md` name (for example, `config-standards.md`); distinct contracts, formats, frameworks, and guides remain descriptively named. Universal governance executables are bare `scripts/educate.ts`, `scripts/govern.ts`, and `scripts/govern.ts`; domain-specific helpers stay descriptive. This is a convention of the `ki-*` set rather than a requirement on every Agent Skill, so a governance skill outside such a repo is exempt for now (rubric **KI-SHAPE-6**). (ki-agentic-harness README)
