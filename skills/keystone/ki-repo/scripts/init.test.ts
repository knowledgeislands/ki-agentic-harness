...
function runInitEnv(dir: string, env: NodeJS.ProcessEnv): { code: number; out: string } {
  return run(INIT, [dir, '--scaffold-config-only'], env)
}
...
// INIT publication refuses symlink leaves, including dangling symlinks, without
...
try {
  ...
  check('INIT symlink config leaf → refuses', result.code !== 0)
  check('INIT symlink config leaf → preserves outside bytes', readFileSync(outsideConfig, 'utf8') === '# outside config\n')
  check('INIT symlink config leaf → remains a symlink', lstatSync(join(symlinkLeaf, '.ki-config.toml')).isSymbolicLink())
  check('INIT symlink config leaf → no transaction artifacts', transactionArtifacts(symlinkLeaf).length === 0)
} finally {
...
try {
  ...
  check('INIT dangling config leaf → refuses', result.code !== 0)
  check('INIT dangling config leaf → remains a symlink', lstatSync(join(danglingLeaf, '.ki-config.toml')).isSymbolicLink())
  check('INIT dangling config leaf → no transaction artifacts', transactionArtifacts(danglingLeaf).length === 0)
} finally {
...
try {
  ...
  check('INIT directory config leaf → refuses', result.code !== 0)
  check('INIT directory config leaf → preserves directory', lstatSync(join(directoryLeaf, '.ki-config.toml')).isDirectory())
  check('INIT directory config leaf → no transaction artifacts', transactionArtifacts(directoryLeaf).length === 0)
} finally {
...
try {
  if (!MKFIFO) check('INIT special config leaf → skipped: mkfifo unavailable', true)
  else {
    execFileSync(MKFIFO, [join(specialLeaf, '.ki-config.toml')])
    const result = runInit(specialLeaf)
    check('INIT special config leaf → refuses', result.code !== 0)
    check('INIT special config leaf → preserves special file', lstatSync(join(specialLeaf, '.ki-config.toml')).isFIFO())
    check('INIT special config leaf → no transaction artifacts', transactionArtifacts(specialLeaf).length === 0)
  }
} finally {
...
try {
  ...
  check('INIT config content race → refuses', result.code !== 0)
  check(
    'INIT config content race → preserves concurrent bytes',
    readFileSync(join(contentRace, '.ki-config.toml'), 'utf8') === '# concurrent config\n'
  )
  check('INIT config content race → no transaction artifacts', transactionArtifacts(contentRace).length === 0)
} finally {
...
try {
  ...
  check('INIT same-byte/new-inode race → refuses', result.code !== 0)
  check('INIT same-byte/new-inode race → preserves replacement bytes', readFileSync(config, 'utf8') === authoringOnlyText)
  check('INIT same-byte/new-inode race → detects the new inode', lstatSync(config).ino !== originalInode)
  check('INIT same-byte/new-inode race → preserves displaced original', lstatSync(`${config}.ki-test-original`).ino === originalInode)
  check('INIT same-byte/new-inode race → no transaction artifacts', transactionArtifacts(sameByteRace).length === 0)
} finally {
...
try {
  ...
  check('INIT post-publication failure → refuses', result.code !== 0)
  check('INIT post-publication failure → restores exact bytes', readFileSync(config, 'utf8') === authoringOnlyText)
  check('INIT post-publication failure → restores exact mode', (lstatSync(config).mode & 0o7777) === 0o600)
  check('INIT post-publication failure → restores exact inode', lstatSync(config).ino === inode)
  check('INIT post-publication failure → no transaction artifacts', transactionArtifacts(exactRollback).length === 0)
} finally {
...
try {
  ...
  check('INIT absent config rollback → refuses', result.code !== 0)
  check('INIT absent config rollback → restores absence', !existsSync(join(absentRollback, '.ki-config.toml')))
  check('INIT absent config rollback → no transaction artifacts', transactionArtifacts(absentRollback).length === 0)
} finally {
...
try {
  ...
  check('INIT rollback publication race → refuses', result.code !== 0)
  check(
    'INIT rollback publication race → replacement survives',
    readFileSync(config, 'utf8') === '# rollback replacement\n' && lstatSync(config).ino !== originalInode
  )
  check('INIT rollback publication race → preserves one private transaction', transactions.length === 1)
  if (transactions.length === 1) {
    ...
    check(
      'INIT rollback publication race → exact authored inode remains quarantined',
...
try {
  ...
  check('INIT post-publication same-byte/new-inode replacement → refuses', result.code !== 0)
  check(
    'INIT post-publication same-byte/new-inode replacement → replacement survives',
  ...
  check('INIT post-publication same-byte/new-inode replacement → preserves private transaction', transactions.length === 1)
  if (transactions.length === 1) {
    const transaction = join(postPublicationReplacement, transactions[0])
    const quarantines = readdirSync(transaction).filter((name) => name.endsWith('.original'))
    check(
      'INIT post-publication same-byte/new-inode replacement → exact authored original survives',
      quarantines.length === 1 &&
        lstatSync(join(transaction, quarantines[0])).ino === originalInode &&
        readFileSync(join(transaction, quarantines[0]), 'utf8') === authoringOnlyText
    )
...
try {
  ...
  check('INIT umask 077 absent config → exits cleanly', result.code === 0)
  check('INIT umask 077 absent config → publishes canonical 0644 mode', (lstatSync(config).mode & 0o7777) === 0o644)
  check('INIT umask 077 absent config → no transaction artifacts', transactionArtifacts(absentUmask).length === 0)
} finally {
...
try {
  ...
  check('INIT umask 0777 absent config → exits cleanly', result.code === 0)
  check('INIT umask 0777 absent config → transaction remains operable and publishes 0644', (lstatSync(config).mode & 0o7777) === 0o644)
  check('INIT umask 0777 absent config → no transaction artifacts', transactionArtifacts(opaqueCreationUmask).length === 0)
} finally {
...
try {
  ...
  check('INIT umask 077 authored 0664 config → exits cleanly', result.code === 0)
  check('INIT umask 077 authored 0664 config → preserves mode', (lstatSync(config).mode & 0o7777) === 0o664)
  check('INIT umask 077 authored 0664 config → preserves authored prefix', readFileSync(config, 'utf8').startsWith(authoringOnlyText))
  check('INIT umask 077 authored 0664 config → no transaction artifacts', transactionArtifacts(authoredMode).length === 0)
} finally {
...
try {
  ...
  check('INIT transaction validation failure → refuses', result.code !== 0)
  check('INIT transaction validation failure → config remains absent', !existsSync(join(transactionValidationFailure, '.ki-config.toml')))
  check(
    'INIT transaction validation failure → created directory is cleaned',
...
try {
  ...
  check('INIT post-fchmod failure → refuses', result.code !== 0)
  check('INIT post-fchmod failure → config remains absent', !existsSync(join(postFchmodFailure, '.ki-config.toml')))
  check('INIT post-fchmod failure → refreshed journal cleans transaction', transactionArtifacts(postFchmodFailure).length === 0)
} finally {
...
try {
  ...
  check('INIT candidate materialisation failure → refuses', result.code !== 0)
  check('INIT candidate materialisation failure → config remains absent', !existsSync(join(materialiseFailure, '.ki-config.toml')))
  check(
    'INIT candidate materialisation failure → immediate journal cleans private state',
...
try {
  ...
  check('INIT external hard-link mutation → refuses', result.code !== 0)
  check('INIT external hard-link mutation → restores snapshot bytes', readFileSync(config, 'utf8') === authoringOnlyText)
  check(
    'INIT external hard-link mutation → alias mutation remains visible',
    readFileSync(alias, 'utf8') === 'external hard-link mutation\n'
  )
  check('INIT external hard-link mutation → preserves one private transaction', transactions.length === 1)
  if (transactions.length === 1) {
    const transaction = join(aliasRace, transactions[0])
    check('INIT external hard-link mutation → private transaction mode is 0700', (lstatSync(transaction).mode & 0o7777) === 0o700)
    const quarantines = readdirSync(transaction).filter((name) => name.endsWith('.original'))
    check('INIT external hard-link mutation → changed original remains quarantined', quarantines.length === 1)
    if (quarantines.length === 1) {
      check(
        'INIT external hard-link mutation → quarantine exposes alias bytes',
...
try {
  ...
  check('INIT validation/removal replacement → refuses', result.code !== 0)
  check('INIT validation/removal replacement → restores snapshot bytes', readFileSync(config, 'utf8') === authoringOnlyText)
  check(
    'INIT validation/removal replacement → preserves displaced original inode',
    lstatSync(`${config}.ki-test-original-after-validation`).ino === originalInode
  )
  check('INIT validation/removal replacement → preserves private evidence', transactions.length === 1)
  if (transactions.length === 1) {
    const transaction = join(removalRace, transactions[0])
    const quarantines = readdirSync(transaction).filter((name) => name.endsWith('.original'))
    check(
      'INIT validation/removal replacement → moved replacement identity and bytes survive',
      quarantines.length === 1 &&
        lstatSync(join(transaction, quarantines[0])).ino !== originalInode &&
...
try {
  ...
  check('INIT symlinked path component → resolves to physical target', result.code === 0)
  check('INIT symlinked path component → publishes only in physical target', existsSync(join(physicalTarget, '.ki-config.toml')))
} finally {
...
try {
  ...
  check('INIT root continuity race → refuses', result.code !== 0)
  check(
    'INIT root continuity race → preserves bound-root config',
    readFileSync(join(displacedRoot, '.ki-config.toml'), 'utf8') === authoringOnlyText
  )
  check('INIT root continuity race → replacement root receives no config', !existsSync(join(rootRace, '.ki-config.toml')))
  check('INIT root continuity race → no artifacts in bound root', transactionArtifacts(displacedRoot).length === 0)
  check('INIT root continuity race → no artifacts in replacement root', transactionArtifacts(rootRace).length === 0)
} finally {
