...
Every declared governance root also commits the repository to a complete **local capability**: the generated manifest must contain EDUCATE, AUDIT, and CONFORM payloads for it — regular files for an ordinary consumer, or contained canonical links for a source harness. `ki-repo` checks this mechanically and offline as `CAPABILITY-COMPLETE`, reading table presence only. Process skills (`ki-delegate`, `ki-next`, `ki-plan`, `ki-recap`, and `ki-repo-review`) and the `ki-bootstrap` chain-starter are global process tooling, not target-local governance contracts; do not declare them in `.ki-config.toml`.
