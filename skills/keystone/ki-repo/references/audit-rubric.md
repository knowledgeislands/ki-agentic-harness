...
- **ki-config [M]** `` `FILES-1` `` `.ki-config.toml` present with an exact `[ki-repo]` root marker (and read for `visibility` + the `[…checks]` override table). A dotted sub-table such as `[ki-repo.checks]` alone does not satisfy the root marker. INIT and CONFORM append the canonical root block when absent without rewriting existing bytes. (Layer 1)
