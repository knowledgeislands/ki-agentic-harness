...
3. Activation is deliberately separate from configuration: the planned native `ki skill add <skill> --scope repo|global` flow creates only managed runtime discovery links after ownership and containment checks. It does not change the declaration into an execution payload, and the native delivery is not implemented by the current local educator scripts.
