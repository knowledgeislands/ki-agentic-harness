...
2. Scaffold the marker: `bun scripts/audit.ts --init >> .ki-config.toml`, then set `visibility` and any `[…checks]` overrides (see [the `.ki-config.toml` contract](references/ki-config-standard.md)). Its presence is what makes the repo compliant.
