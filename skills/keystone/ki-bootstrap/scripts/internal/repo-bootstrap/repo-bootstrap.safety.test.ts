...
try {
  ...
  check('--help → exits successfully and renders usage', result.status === 0 && result.stdout.includes('usage: repo-bootstrap.ts'))
...
for (const [label, rel] of [
  ...
  ['HELP snapshot', 'bootstrap/checkers/ki-authoring/help.md']
] as const) {
  const root = fixture()
  try {
...
try {
  ...
  check('staged mutation after journaling → no destination generation is published', !existsSync(join(stagedMutation, '.ki', 'bin')))
  check(
...
try {
  ...
  check('bound root inode replacement → replacement root receives no .ki writes', !existsSync(join(rootReplacement, '.ki')))
} finally {
...
try {
  ...
  check('root replacement during config scaffold → replacement root receives no .ki writes', !existsSync(join(scaffoldReplacement, '.ki')))
} finally {
...
for (const [name, env] of [
  ...
  try {
    ...
    check(`raced .ki ${name} → bootstrap refuses`, result.status !== 0)
    check(`raced .ki ${name} → third-party directory is preserved`, lstatSync(join(root, '.ki')).isDirectory())
  } finally {
    rmSync(root, { recursive: true, force: true })
  }
}
...
try {
  mkdirSync(join(stagingCollision, '.ki', '.bootstrap-staging-fixed'), { recursive: true })
  writeFileSync(join(stagingCollision, '.ki', '.bootstrap-staging-fixed', 'sentinel'), 'preserve\n')
...
try {
  mkdirSync(join(publicMeta, '.ki'), { mode: 0o777 })
  chmodSync(join(publicMeta, '.ki'), 0o777)
  const before = snapshot(publicMeta)
  const result = run(publicMeta)
  check('group/other-writable .ki → bootstrap refuses', result.status !== 0)
  check('group/other-writable .ki → target remains unchanged', snapshot(publicMeta) === before)
} finally {
...
try {
  check('greenfield absent .ki → complete generation succeeds', run(greenfield).status === 0)
  check('greenfield success → staging and lock are removed', transactionArtifacts(greenfield).length === 0)
  mkdirSync(join(greenfield, '.ki', 'audits'))
  writeFileSync(join(greenfield, '.ki', 'audits', 'report.json'), '{"preserve":true}\n')
  const audits = snapshot(join(greenfield, '.ki', 'audits'))
  mkdirSync(join(greenfield, '.ki', 'bootstrap', 'checkers', 'obsolete'))
  writeFileSync(join(greenfield, '.ki', 'bootstrap', 'checkers', 'obsolete', 'audit.ts'), 'obsolete\n')
  check('successful re-bootstrap → succeeds', run(greenfield).status === 0)
  check('successful re-bootstrap → obsolete checker is pruned', !existsSync(join(greenfield, '.ki', 'bootstrap', 'checkers', 'obsolete')))
  check('successful re-bootstrap → audits are byte-for-byte preserved', snapshot(join(greenfield, '.ki', 'audits')) === audits)
} finally {
  rmSync(greenfield, { recursive: true, force: true })
}

const legacyRootMigration = fixture()
try {
  check('legacy root fixture → initial bootstrap succeeds', run(legacyRootMigration).status === 0)
  renameSync(join(legacyRootMigration, '.ki'), join(legacyRootMigration, '.ki-meta'))
  const manifestPath = join(legacyRootMigration, '.ki-meta', 'manifest.json')
  writeFileSync(manifestPath, readFileSync(manifestPath, 'utf8').replaceAll('.ki/', '.ki-meta/'))
  check('proven legacy root → re-bootstrap migrates cleanly', run(legacyRootMigration).status === 0)
  check('proven legacy root → old generated tree is removed', !existsSync(join(legacyRootMigration, '.ki-meta')))
  check('proven legacy root → direct .ki generation is published', existsSync(join(legacyRootMigration, '.ki', 'manifest.json')))
} finally {
  rmSync(legacyRootMigration, { recursive: true, force: true })
}

const unsafeLegacyRoot = fixture()
try {
  check('unsafe legacy root fixture → initial bootstrap succeeds', run(unsafeLegacyRoot).status === 0)
  renameSync(join(unsafeLegacyRoot, '.ki'), join(unsafeLegacyRoot, '.ki-meta'))
  const manifestPath = join(unsafeLegacyRoot, '.ki-meta', 'manifest.json')
  writeFileSync(manifestPath, readFileSync(manifestPath, 'utf8').replaceAll('.ki/', '.ki-meta/'))
  mkdirSync(join(unsafeLegacyRoot, '.ki-meta', 'unproven'))
  const before = snapshot(join(unsafeLegacyRoot, '.ki-meta'))
  check('unsafe legacy root → bootstrap preserves it for manual recovery', run(unsafeLegacyRoot).status !== 0)
  check('unsafe legacy root → unproven content is unchanged', snapshot(join(unsafeLegacyRoot, '.ki-meta')) === before)
} finally {
  rmSync(unsafeLegacyRoot, { recursive: true, force: true })
}

const legacyMigration = fixture()
try {
  check('legacy checker-layout fixture → initial bootstrap succeeds', run(legacyMigration).status === 0)
  const meta = join(legacyMigration, '.ki')
...
try {
  ...
  const meta = join(staleLegacy, '.ki')
  renameSync(join(meta, 'bootstrap', 'checkers'), join(meta, 'skills'))
  const manifestPath = join(meta, 'manifest.json')
  writeFileSync(manifestPath, readFileSync(manifestPath, 'utf8').replaceAll('.ki/bootstrap/checkers/', '.ki/skills/'))
...
try {
  mkdirSync(join(symlinkedLegacy, '.ki'))
  writeFileSync(join(symlinkedLegacyOutside, 'sentinel'), 'outside\n')
  symlinkSync(symlinkedLegacyOutside, join(symlinkedLegacy, '.ki', 'skills'))
...
for (const [label, leaf] of [
  ['checkers', join('bootstrap', 'checkers')],
  ['educators', join('bootstrap', 'educators')],
  ['bin', 'bin'],
  ...
  try {
    mkdirSync(join(root, '.ki'))
    writeFileSync(join(outside, 'sentinel'), 'outside\n')
    mkdirSync(dirname(join(root, '.ki', leaf)), { recursive: true })
    symlinkSync(label === 'manifest.json' ? join(outside, 'missing') : outside, join(root, '.ki', leaf))
    const before = snapshot(outside)
    const result = run(root)
    check(`symlinked ${label} → bootstrap refuses`, result.status !== 0)
    check(`symlinked ${label} → outside remains unchanged`, snapshot(outside) === before)
    check(`symlinked ${label} → no transaction artifacts remain`, transactionArtifacts(root).length === 0)
  } finally {
    rmSync(root, { recursive: true, force: true })
...
try {
  ...
  symlinkSync(metaOutside, join(metaSymlink, '.ki'))
  const before = snapshot(metaOutside)
  check('symlinked .ki → bootstrap refuses', run(metaSymlink).status !== 0)
  check('symlinked .ki → outside remains unchanged', snapshot(metaOutside) === before)
} finally {
...
try {
  mkdirSync(join(locked, '.ki'))
  writeFileSync(join(locked, '.ki', '.bootstrap.lock'), 'other invocation\n')
...
try {
  ...
  const aggregate = join(prePublication, '.ki', 'bin', 'aggregate.ts')
...
try {
  ...
  writeFileSync(join(postCheckSwap, '.ki', 'bin', 'aggregate.ts'), 'stale aggregate\n')
  ...
  check(
    ...
    readFileSync(join(postCheckSwap, '.ki', 'bin', 'aggregate.ts'), 'utf8') === 'third-party post-check replacement\n'
  )
  check(
    'post-quarantine destination recreation → old bytes remain quarantined',
    snapshot(join(postCheckSwap, '.ki')).includes(Buffer.from('stale aggregate\n').toString('base64'))
  )
  check('post-quarantine destination recreation → exact conflict is reported', output.includes('.ki/bin/aggregate.ts'))
} finally {
...
try {
  ...
  const aggregate = join(quarantineSnapshotFailure, '.ki', 'bin', 'aggregate.ts')
...
for (const [label, envName] of [
  ...
  try {
    ...
    const aggregate = join(root, '.ki', 'bin', 'aggregate.ts')
    const generated = readFileSync(aggregate)
    writeFileSync(aggregate, 'stale aggregate\n')
    writeFileSync(join(root, '.ki', 'manifest.json'), '{"stale":true}\n')
...
try {
  ...
  const obsolete = join(rollbackMode, '.ki', 'bootstrap', 'checkers', 'obsolete')
  mkdirSync(obsolete, { mode: 0o711 })
  chmodSync(obsolete, 0o711)
  writeFileSync(join(obsolete, 'audit.ts'), 'obsolete\n')
  writeFileSync(join(rollbackMode, '.ki', 'bin', 'aggregate.ts'), 'stale aggregate\n')
  writeFileSync(join(rollbackMode, '.ki', 'manifest.json'), '{"stale":true}\n')
...
try {
  ...
  const aggregate = join(hardLinkRollback, '.ki', 'bin', 'aggregate.ts')
  const alias = join(hardLinkRollback, 'aggregate-alias.ts')
  writeFileSync(aggregate, 'stale aggregate\n')
  linkSync(aggregate, alias)
  writeFileSync(join(hardLinkRollback, '.ki', 'manifest.json'), '{"stale":true}\n')
...
try {
  ...
  check(
    ...
    readFileSync(join(lockReplacement, '.ki', '.bootstrap.lock'), 'utf8') === 'third-party lock replacement\n'
  )
...
try {
  ...
  writeFileSync(join(rollbackConflict, '.ki', 'bin', 'aggregate.ts'), 'stale aggregate\n')
  ...
  const output = `${result.stdout ?? ''}${result.stderr ?? ''}`
  check('third-party rollback mutation → exact conflict is reported', result.status !== 0 && output.includes('.ki/bin/aggregate.ts'))
  check(
    'third-party rollback mutation → changed destination is preserved',
    readFileSync(join(rollbackConflict, '.ki', 'bin', 'aggregate.ts'), 'utf8') === 'third-party rollback conflict\n'
  )
