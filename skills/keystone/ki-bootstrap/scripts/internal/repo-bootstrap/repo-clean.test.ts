...
try {
  ...
  check('CLEAN dry-run succeeds and reports generated state', dryRun.status === 0 && dryRun.stdout.includes('.ki-meta'))
  check('CLEAN dry-run leaves every byte unchanged', snapshot(ordinary) === beforeDryRun)
  check(
    'CLEAN removes manifest-owned metadata and regular runtime copies',
    runClean(ordinary).status === 0 && !existsSync(join(ordinary, '.ki-meta'))
  )
  check(
  ...
  check(
    ...
    educate(ordinary).status === 0 &&
      existsSync(join(ordinary, '.ki-meta', 'manifest.json')) &&
...
const unfamiliarMeta = fixture()
try {
  check('unfamiliar metadata fixture EDUCATE succeeds', educate(unfamiliarMeta).status === 0)
  writeFileSync(join(unfamiliarMeta, '.ki-meta', 'authored-note'), 'preserve\n')
  const before = snapshot(unfamiliarMeta)
  check('CLEAN refuses metadata with unfamiliar content', runClean(unfamiliarMeta).status !== 0)
  check('CLEAN refusal leaves unfamiliar metadata untouched', snapshot(unfamiliarMeta) === before)
} finally {
  rmSync(unfamiliarMeta, { recursive: true, force: true })
}
...
try {
  ...
  check('CLEAN detects a mutation before deletion', result.status !== 0 && existsSync(join(raced, '.ki-meta')))
