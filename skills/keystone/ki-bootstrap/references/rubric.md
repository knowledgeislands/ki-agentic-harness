...
- **BOOT-1 [M] — runtime skill directories mirror declared coverage** — For each declared runtime, the project-local skills directory mirrors the repository's declared coverage. Ordinary repositories receive complete generated regular-file copies; a harness links its own canonical source skills. A repository-local `.ki-self/` source projects by relative link into every declared runtime. Missing, extra, invalidly linked, or source-drifted payloads are WARN; an unresolvable declaration or dependency is FAIL and must be reconciled by hand. ([KR], [AH], [ADR-KI-HARNESS-011])
