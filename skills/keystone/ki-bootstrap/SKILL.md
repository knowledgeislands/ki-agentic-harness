...
- **Greenfield:** a repo with no `.ki-config.toml` enters through `ki-repo`'s init — `bun scripts/bootstrap.ts <target> --seed ki-repo` (or `bun skills/ki-repo/scripts/init.ts <target>`) vendors `ki-repo` so its INIT is reachable to scaffold the config; re-run bare for the full declared set once the coverage tables (foundations included) exist. There is no injected baseline — bare `bootstrap.ts <target>` on an empty config resolves to the empty set.
