...
- **Reference-implementation gap.** The skill has no live, conformant reference base. Either conform arcadia-principal's Activities notes (add `realization`, switch status to the enum, add a `[ki-activities]` table) so the citation is honest, or reclassify the citation as a pre-adoption sample. Decide direction before next REFRESH.
...
- **Legacy skill-name rename.** arcadia-principal still uses `knowledgeislands-*` config names; when it adopts `ki-activities` it should use the `ki-*` name. Base-migration concern, tracked here only because it blocks the reference base from becoming conformant.
