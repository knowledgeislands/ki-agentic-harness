...
1. Determine the `decision_type` (one of the nine values) and derive the prefix from the table in `SKILL.md`. Determine the `SCOPE` from `.ki-config.toml` (e.g. `ARCADIA`). Derive the next available `NNN` from existing files — serials are global within the scope across all prefixes, so scan all `<PREFIX>-<SCOPE>-NNN` files together.
2. Write the DR using [the template](dr-format.md#template) — all five Nygard sections (Title, Status/Date, Context, Decision, Consequences) plus an optional `## References` section. KB repos require frontmatter (`type: admin/governance/decision`, `decision_type`, `status`, `author`). Status starts as `Proposed`.
