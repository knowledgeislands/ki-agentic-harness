...
- **`TAP-DESC-STYLE` is a mirror.** The ≤ 80-char + no-leading-article rule is copied from `brew style` so the check fires without `brew` installed. If that cop changes upstream, update the mirror in `audit-homebrew-tap.ts` or it will disagree with `brew`.
