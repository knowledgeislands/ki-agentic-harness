...
4. **Reconcile the mirrored check.** `TAP-DESC-STYLE` mirrors a `brew style` rule so it fires without `brew` on PATH — if that rule changed upstream, update the mirror in `audit-homebrew-tap.ts` to match, or the local check will disagree with `brew`.
