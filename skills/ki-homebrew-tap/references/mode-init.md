# Mode INIT — scaffold a new Homebrew tap

_On-demand procedure for ki-homebrew-tap's INIT mode. The canonical shape and the name constraint live in [`SKILL.md`](../SKILL.md) and are already loaded; this file is the procedure only._
...
4. **Wire governance.** Add `[ki-repo]` + `[ki-homebrew-tap]` to `.ki-config.toml` — `bun ../scripts/audit-homebrew-tap.ts --init` prints the keyless `[ki-homebrew-tap]` block. Run `ki-repo`'s INIT for the repo-level files and GitHub settings. Do **not** add `[ki-engineering]` — a tap has no TypeScript toolchain (the `ki-plugins` precedent).
5. **Verify.** Run the checker (`bun ../scripts/audit-homebrew-tap.ts <path>`) to 0 FAIL, and — if `brew` is installed — `brew style` + `brew audit --strict` on each formula. Confirm `brew install <owner>/<x>/<tool>` works before publishing.
