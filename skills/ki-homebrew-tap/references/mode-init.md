...
4. **Wire governance.** Add `[ki-repo]` + `[ki-homebrew-tap]` to `.ki-config.toml` — `bun ../scripts/audit.ts --init` prints the keyless `[ki-homebrew-tap]` block. Run `ki-repo`'s INIT for the repo-level files and GitHub settings. Do **not** add `[ki-engineering]` — a tap has no TypeScript toolchain (the `ki-plugins` precedent).
