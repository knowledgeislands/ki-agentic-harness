...
3. Add the `[ki-homebrew-tap]` marker (and `[ki-repo]`) to `.ki-config.toml` if absent — `bun ../scripts/audit-homebrew-tap.ts --init` prints the block. Run `ki-repo`'s CONFORM for the repo-level files/settings.
