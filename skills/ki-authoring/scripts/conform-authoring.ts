 ...
 * thin write-pass twin of audit-authoring.ts's mechanical half.
 *
 *   bun scripts/conform-authoring.ts [path]   # default: cwd
 *   --dry-run                                  # check-mode only, write nothing
 *
 * Deliberately a THIN wrapper: it shells out to the exact same Prettier +
 * markdownlint-cli2 commands `ki:lint:md` / `ki:lint:md:check` already run —
...
// Same commands package.json's ki:lint:md / ki:lint:md:check wrap — write mode
...
console.log(
  `\n${paint(C.dim, 'mechanical layer applied — re-run `bun scripts/audit-authoring.ts ' + target + '` (or `ki:authoring:audit`) to confirm findings clear.')}`
)
