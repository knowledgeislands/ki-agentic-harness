...
A **governance skill** — one that holds a house standard — exposes a common mode set so a reader moves between skills without relearning: the universal four are **AUDIT** (check an artifact against the standard), **CONFORM** (bring an existing artifact into line), **INIT** (scaffold a new conformant artifact, or bring an off-standard one onto the floor from scratch — its mechanical half a per-skill `scripts/bootstrap.ts`), and **REFRESH** (re-anchor the standard to its sources). Modes beyond the four are skill-specific: operational modes serve a skill's own domain (the `ki-kb-base` note-ops — DIGEST / EXTRACT / QUERY / SAVE / UPDATE), and OPTIMISE pushes a compliant artifact toward excellent. All `ki-*` skills are governance skills on this model. (ki-agentic-harness README)
