...
- **SHAPE-5 [J]** A **governance skill** (one that holds a standard) exposes the universal modes **AUDIT** + **CONFORM** + **INIT** + **REFRESH** — INIT scaffolds a new artifact (or brings an off-standard one onto the floor from scratch), its mechanical half being the per-skill `scripts/init.ts` (the INIT counterpart to `audit-*.ts`, a thin delegator into the central chain engine); any further modes (`OPTIMISE` to push a compliant artifact from the floor toward excellent, and operational modes like kb's note-ops) are skill-specific. Modes are named, not lettered, and ordered alphabetically in the body and `argument-hint`. (ki-agentic-harness README)
...
- **SHAPE-11 [M]** _Exposes the universal HELP mode._ Every governance skill's `argument-hint` lists a `help` verb, so the no-mode default and the `help` / `-h` / `?` pure-explain form are discoverable (ADR-KI-HARNESS-SKILLS-001). The HELP block itself is **generated, not authored** — the shared renderer (`skills/ki-bootstrap/scripts/skill-help.ts`, surfaced as `ki:skills:help <name>`) reads what the `SKILL.md` already declares and injects HELP into every skill's mode list — so a skill's only footprint is this one token plus the INVOKE-1 prose. The linter verifies the `help` token; the prose HELP semantics are INVOKE-1 **[J]**. (ADR-KI-HARNESS-SKILLS-001)
- **SHAPE-12 [M]** _Mode vocabulary is canonical and complete._ A governance skill exposes **AUDIT**, **CONFORM**, **INIT**, **REFRESH** and **HELP** spelled exactly so — a governance skill missing any universal verb from its `argument-hint` (INIT is the common gap) **WARNs**; `NEW`, `OPTIMISE`, and operational verbs are additive, never substitutes for a universal mode (a collection skill exposes both INIT and NEW). The vendoring leg: a governance skill's frontmatter **declares its vendorable mechanical unit** (`vendors:`, beside `implies:`) — a checker path or a command form — so the central bootstrap engine can vendor it into a target's `.ki-meta/`; a missing declaration **WARNs** (aligned with the engine's migration fallback to filename-convention discovery). Process skills are exempt throughout. (ADR-KI-HARNESS-SKILLS-001, ADR-KI-HARNESS-006)
