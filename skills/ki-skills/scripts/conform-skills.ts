 ...
 * lint-skills.ts's findings that are unambiguous and reversible, leaving
 ...
 * dir containing skills), matching the house conform shape (conform-repo.ts,
 * conform-drs.ts) — `bun conform-skills.ts [path]` / `ki:skills:conform`.
 * Detection logic (hasBackslashLink, hintVerbs, isProcessSkill, discoverSkillDirs)
 * is copied from lint-skills.ts — kept in lockstep, never imported, so this
 * script stays valid standalone per the composition-only rule.
 *
 *   bun scripts/conform-skills.ts [path]   # default: cwd
...
// ── kept in lockstep with lint-skills.ts ──
...
function conformSkill(dir: string, dryRun: boolean, todos: string[]): void {
  ...
  // ── SHAPE-12 (vendors leg) — process skills are exempt, same as lint-skills.ts ──
  if (!process) {
...
function main(): void {
  ...
  console.log(
    `  - Everything else lint-skills.ts flags — DESC-1/2/3, SHAPE-13, SIZE-1/2, LINK-2, SHAPE-2/7/8/9, REF-3, COLL-1, LONG-3/4 — is prose/authoring or cross-file judgment, never auto-fixed here.`
  )
  console.log(
    `\n${paint(C.dim, 'mechanical layer applied — re-run `bun scripts/lint-skills.ts` (or `ki:skills:lint`) to confirm findings clear.')}`
  )
}
