...
The normative model `knowledgeislands-bootstrap` governs. It is small by design: this is the **keystone**, the one `knowledgeislands-*` skill kept globally installed, so its standing cost is paid on every turn in every project.
...
That leaves a bootstrap problem: something must be globally available to _wire_ a repo's project-local skills, and to reach `knowledgeislands-repo`'s INIT in a repo that has no `.ki-config.toml` yet. Globalizing the heavy `repo` skill wastes the budget. So a single tiny skill — this one — is the global keystone; it does nothing but mirror a repo's declared coverage into its `.claude/skills/`.

## The invariant

For a Knowledge Islands repo, `.claude/skills/` contains exactly:

> **the skills the repo declares** (`[knowledgeislands-<skill>]` tables in `.ki-config.toml`) **∪ the baseline `knowledgeislands-repo` + `knowledgeislands-authoring`**, minus `knowledgeislands-bootstrap` itself (which is global).

- **Declared coverage** is owned by `knowledgeislands-repo`'s coverage cascade — this skill _reads_ the tables, never edits them. Whether the declared set is correct for the repo is a `knowledgeislands-repo` question.
...
- **Relative symlinks** into the harness's `skills/` (e.g. `.claude/skills/knowledgeislands-mcp -> ../../../arcadia-agentic-harness/skills/knowledgeislands-mcp`), computed for wherever the harness actually sits.
