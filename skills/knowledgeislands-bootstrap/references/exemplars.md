...
A repo opts skills in by adding `[knowledgeislands-<skill>]` tables. The keystone linker reads these tables and mirrors the matching skills from the harness into `.claude/skills/`. Tables with no keys (bare `[knowledgeislands-kb]`) are valid — presence alone is the opt-in signal. From `arcadia-principal/.ki-config.toml`:
...
The keystone linker also always links the baseline (`knowledgeislands-repo` + `knowledgeislands-authoring`) regardless of whether those tables appear — they are cascade-exempt universals.
