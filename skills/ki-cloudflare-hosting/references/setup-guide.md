...
| A built `dist/` produced by `ki-11ty-websites`          | run `bun run ki:site:build` once to confirm |
...
`ki:site:build` and `ki:site:dev` are owned by `ki-11ty-websites` — do not redefine them here.
...
Add the `[ki-cloudflare-hosting]` table so the mechanical checker can find the repo:

```toml
[ki-cloudflare-hosting]
...
```bash
bun /path/to/skills/ki-cloudflare-hosting/scripts/audit-cloudflare-hosting.ts <repo-root>
```
