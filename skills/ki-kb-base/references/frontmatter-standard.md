...
† For example, `ki-activities` uses `active` / `inactive`.

**Freshness** is carried by the timestamps, not by `status`: a note is current while `reviewed` is at or after its last `updated`, and goes stale once `updated` moves ahead of `reviewed` (or `reviewed` is absent). `status` is reserved for a `type`'s own lifecycle where it has one — for example `ki-activities` uses `active` / `inactive`.
...
The `Streams` zone's internal structure is owned by `ki-streams`; these are its note kinds.

| Type               | Path context                                     | Defined by   |
| ------------------ | ------------------------------------------------ | ------------ |
| `streams/zone`     | `Streams/Streams.md` (zone root)                 | `ki-streams` |
| `streams/focus`    | A focus / lifecycle-folder summary (`Active`, …) | `ki-streams` |
| `streams/proposal` | A workstream proposal (the enactment unit)       | `ki-streams` |
| `streams/note`     | A working note within a stream                   | `ki-streams` |
...
| `streams/proposal` | `status` | the Streams proposal lifecycle (values owned by `ki-streams`) | `ki-streams` |
| `streams/proposal` | `priority` | `urgent` / `high` / `medium` / `low` | `ki-streams` |
| `streams/proposal` | `dependencies` | list of blocking stream names (`[]` when none) | `ki-streams` |
