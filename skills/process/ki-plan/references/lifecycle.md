...
2. Run the read-only `ki-repo-roadmap` audit and continue only with zero FAIL and zero WARN. Require every `## Steps` item to be marked complete. Re-run the plan's `## Verify` checks now and retain their concrete commands and results; stop if any check fails or cannot be evidenced. Confirm that every qualified `blocked-by` plan is done.
3. Append one non-empty `## Acceptance` section after `## Dependencies / blocks`, then set `status: acceptance` and regenerate the global index in the same guarded transaction used for execution progress. Record a concise packet with exactly these labelled concerns: **Delivered** (the material outcome), **Verification** (the evidence from step 2), **Outstanding concerns** (or `None`), and **Mini recap** (what was learned plus any proposed route). The mini recap is plan-scoped review evidence, not approval to write its learning anywhere else.
