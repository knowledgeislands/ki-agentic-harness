...
1. Require the thematic profile. Parse the plan identifier as an uppercase theme code plus a zero-padded numeric serial, `<THEME>-<NNN>`. Locate the one `<THEME>-<NNN>-*.md` under the corresponding theme's `plans/` directory; stop on zero or multiple matches.
2. Run the read-only `ki-repo-roadmap` audit and continue only with zero FAIL and zero WARN. Read the plan and resolve its qualified `roadmap: <theme>/<item-slug>` locator to exactly one `Blocking` or `Next` item in `docs/roadmap/<theme>/ROADMAP.md`; stop on any mismatch or ambiguity.
...
7. Report: "Plan `<THEME>-<NNN>` closed."
...
5. When all steps are done, make the final transient `status: done` plus index update through the same transaction, then run `done <THEME>-<NNN>` to close the plan and its canonical roadmap item only after the implementation and verification have landed.
