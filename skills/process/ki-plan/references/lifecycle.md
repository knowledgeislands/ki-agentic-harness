...
1. Require the thematic profile. Locate the one `<NNN>-*.md` under `docs/roadmap/*/plans/` whose global numeric prefix matches `<id>`; stop on zero or multiple matches.
...
2. Require an existing canonical `docs/roadmap/<theme>/ROADMAP.md` in the thematic profile. Confirm the exact qualified `roadmap: <theme>/<item-slug>` locator this plan executes; it must resolve to one `Blocking` or `Next` item in that theme roadmap. Confirm `blocks` / `blocked-by` (global ids or `—`). Infer from context and confirm; do not ask for a phase.
3. Run the read-only `ki-project-roadmap` audit and continue only with zero FAIL and zero WARN. Read `docs/roadmap/README.md` and every plan under every theme. Next id = highest validated global id + 1, zero-padded to at least three digits; use `001` if none exists. Stop on index/disk disagreement, malformed ids, duplicates, broken links, or dependency drift.
...
6. Form the id union from the validated index ids and on-disk filename/frontmatter ids. The candidate id is its maximum plus one, padded to at least three digits; use `001` when both sources are empty. Do not allocate from only one source.
