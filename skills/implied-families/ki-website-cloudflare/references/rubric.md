...
Severity: **FAIL** (ship-stopper — the site can't deploy, or deploys the wrong way), **WARN** (config / script divergence), **POLISH** (domains / CI / consistency) — the shared ladder, defined in `ki-skills`' [checker contract](../../../general-governance/ki-skills/references/checker-contract.md).
