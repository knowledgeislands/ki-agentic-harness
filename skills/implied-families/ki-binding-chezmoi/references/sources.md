...
**REFRESH, 2026-07-14** — BINDCHEZ-3 broadened to recognise a second source-data shape: the _inverted_ pattern, where the repo carries a plain, non-templated, chezmoi-managed source file applied verbatim to the canonical XDG path, and the render template reads it via chezmoi's `include` from the source tree (never `.chezmoidata`, never a shell-out to the applied `$HOME` copy — that has a fresh-machine bootstrap race `include` avoids). Triggered by the `~/.local/share/chezmoi` dotfiles repo migrating its MCP source from `.chezmoidata/mcps.yaml` to `dot_config/ki/mcp-servers.yaml`; standard, rubric, and checker were updated together per BINDCHEZ-7. Verified against that repo post-migration: BINDCHEZ-3 now PASSes as `inverted` pattern.
