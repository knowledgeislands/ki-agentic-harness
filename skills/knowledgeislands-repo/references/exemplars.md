...
reference set for public repos (topics, secret scanning, MIT license). For the full source list and last-review dates, see
...
Every KI-governed repo carries a `.ki-config.toml` at its root. The file is the compliance marker; its presence is what the coverage cascade
gates on. A repo that fully conforms to org defaults needs only the `[knowledgeislands-repo]` table with a `visibility` declaration and
silence under `[knowledgeislands-repo.checks]` (or no checks table at all). Each additional governance skill the repo opts into adds its own
table — the tables are independent, each skill reads only its own.
...
# No [knowledgeislands-repo.checks] needed: this repo takes all org defaults.
...
`branch-protection` defaults **off** for all KI repos — `main` is open, direct pushes allowed. A repo that wants a protected `main` declares
it with a single boolean. When `branch-protection = true`, the auditor requires: a PR (0 approvals), the `build` status check, linear
history, no force-push, no deletion, admins not enforced. No other check is affected by this override.
...
The harness `CLAUDE.md` is the pattern for all KI repos: a short orientation paragraph, then `@`-imports for per-topic files rather than one
monolithic document. Topic files can be updated independently and loaded selectively. The imports section should come early so an agent
session loads the topic context before the repo-specific prose. Per the skills rubric (SHAPE-7), every KI repo carries a `CLAUDE.md` — the
field-name convention shown here (`@memory-scope.md`, `@workflow.md`) is the global user pattern; repo-level files typically import
repo-specific topics (`@toolchain.md`, `@conventions.md`).
...
When a repo carries a four-part agentic harness layout (`skills/`, `agents/`, `mcp/`, `evals/`), its `.ki-config.toml` must declare all four
relevant tables so `knowledgeislands-repo`'s coverage cascade does not warn on detected-but-undeclared artifacts. The harness repo
(`arcadia-agentic-harness`) is the canonical example — it opts into every applicable governance skill and documents why each table is
present.
