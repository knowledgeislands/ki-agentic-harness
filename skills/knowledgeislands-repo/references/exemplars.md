...
Every KI-governed repo carries a `.ki-config.toml` at its root. The file is the compliance marker; its presence is what the coverage cascade gates on. A repo that fully conforms to org defaults needs only the `[knowledgeislands-repo]` table with a `visibility` declaration and silence under `[knowledgeislands-repo.checks]` (or no checks table at all). Each additional governance skill the repo opts into adds its own table — the tables are independent, each skill reads only its own.
...
When a repo carries a four-part agentic harness layout (`skills/`, `agents/`, `mcp/`, `evals/`), its `.ki-config.toml` must declare all four relevant tables so `knowledgeislands-repo`'s coverage cascade does not warn on detected-but-undeclared artifacts. The harness repo (`arcadia-agentic-harness`) is the canonical example — it opts into every applicable governance skill and documents why each table is present.
