...
Its **presence is the marker of a Knowledge Islands–compliant repo**, and the **gate of the coverage cascade** (below): a repo that carries
`.ki-config.toml` has opted into the house standards, and the standard-holding skills are what hold it to them, each reading its own table
where it needs declared config. Onboarding a repo (adding the file) is the act of making it compliant; `knowledgeislands-repo` requires it
as a Layer-1 root file, is the skill that audits it, and — because it is the gate — is also the skill that checks the repo declares the
other standards that govern it (_Coverage enforcement_, below).
...
A `[knowledgeislands-<skill>]` table plays one or both of two roles:
...
The two are separable: a base on the canonical zone names declares a bare `[knowledgeislands-kb]` (marker only, no keys); a base that
renames a zone adds a `[knowledgeislands-kb.zones]` alias (config). The marker/opt-in skills are `knowledgeislands-engineering`, `-kb`,
`-streams`, `-11ty-websites`, `-cloudflare-hosting`, `-mcp`, `-skills`, and `-agents`. `knowledgeislands-repo` is the **bedrock marker** —
the file's very presence is what makes the repo a ki-repo — and `knowledgeislands-authoring` is **universal** (it governs every markdown
repo), so neither needs a separate opt-in.

So **what an absent table means is per-skill**, and that is exactly what _Coverage enforcement_ (below) checks:

| Table absent                         | Means                                                                                     |
| ------------------------------------ | ----------------------------------------------------------------------------------------- |
| `[knowledgeislands-repo]` (the file) | not a ki-repo — the marker itself (bedrock; missing file is a FAIL)                       |
| any other marker skill's table       | not opted into that standard — a coverage WARN _if_ the repo shows that skill's artifacts |
| `knowledgeislands-authoring`         | nothing — universal, no opt-in table exists                                               |
...
The file's presence is the **gate of an audit cascade**. Once a repo is confirmed a ki-repo (it carries `.ki-config.toml`),
`knowledgeislands-repo`'s auditor checks that the repo **declares an opt-in table for every governance skill whose applicability is
detectable in it**. A detected-but-undeclared signal WARNs ("looks governed by `knowledgeislands-<skill>` but declares no
`[knowledgeislands-<skill>]`"); a declared-but-undetected table WARNs as a possibly stale opt-in.

The gate is what prevents a **false positive**: a plain git repo that has, say, an `eleventy.config` but **no `.ki-config.toml`** is not a
ki-repo, so it is never told to declare a website table. It simply takes the `ki-config` required-file FAIL. Coverage is only ever
considered _after_ the marker confirms a ki-repo.

The detection signals `knowledgeislands-repo` uses (one recursive tree read + `package.json`):

| Skill                                 | Detection signal                            | Opt-in table                            |
| ------------------------------------- | ------------------------------------------- | --------------------------------------- |
| `knowledgeislands-engineering`        | `package.json` present                      | `[knowledgeislands-engineering]`        |
| `knowledgeislands-kb`                 | canonical zones (`Pillars/` + `Resources/`) | `[knowledgeislands-kb]`                 |
| `knowledgeislands-streams`            | `Streams/` zone                             | `[knowledgeislands-streams]`            |
| `knowledgeislands-11ty-websites`      | `eleventy.config.*`                         | `[knowledgeislands-11ty-websites]`      |
| `knowledgeislands-cloudflare-hosting` | a `wrangler.*` config                       | `[knowledgeislands-cloudflare-hosting]` |
| `knowledgeislands-mcp`                | `@modelcontextprotocol/sdk` dependency      | `[knowledgeislands-mcp]`                |
| `knowledgeislands-skills`             | `skills/*/SKILL.md`                         | `[knowledgeislands-skills]`             |
| `knowledgeislands-agents`             | `agents/**/*.md`                            | `[knowledgeislands-agents]`             |

This is the **one place** `knowledgeislands-repo` reads across skill tables — and it reads only table **presence**, never another skill's
keys (_validate down, ignore across_ still governs table _contents_). It is an **audit-time enforcement** run by `repo`'s auditor, not
behaviour baked into the regular use of each skill. A repo opts out of a single signal it doesn't want enforced with a
`coverage-<skill> = false` entry in its `[knowledgeislands-repo.checks]` table (e.g. a repo that vendors an `eleventy.config` it does not
own) — reported as an acknowledged note.

No marker table is decorative — each is read by code. Most are read by their **owning** skill's auditor too
(`-engineering`/`-kb`/`-streams`/`-11ty-websites`/`-cloudflare-hosting`/`-mcp` each read their own table when run).
`knowledgeislands-skills` and `knowledgeislands-agents` are the documented exception: their checkers lint an artifact set (`SKILL.md` files,
agent definitions), not a repo's config, so their opt-in table is read only by `knowledgeislands-repo`'s coverage check.
