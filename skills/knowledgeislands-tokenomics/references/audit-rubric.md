...
- **CFG-1 [M]** The `[knowledgeislands-tokenomics]` table is parsed and **validated down** — an unrecognised key WARNs; a malformed budget value (non-numeric) **FAILs**; another skill's table is never read.
...
- **CFG-4 [M]** `preferred_model` is **declared** in the `[knowledgeislands-tokenomics]` table — its tier value is checked mechanically; whether the chosen tier is appropriate for the work stays judgment (RUN-2).
