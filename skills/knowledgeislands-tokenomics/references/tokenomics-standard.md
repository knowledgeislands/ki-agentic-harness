...
**The config table.** A target opts in with a `[knowledgeislands-tokenomics]` table in its `.ki-config.toml`, read **validate-down**: the checker validates only its own table, WARNs on a key it does not recognise, and never reads another skill's table. Shape (all keys optional; `--init` scaffolds them):
