...
2. Trim the biggest line items: lift rarely-read detail out of `CLAUDE.md` into on-demand files (or exclude an irrelevant ancestor
   `CLAUDE.md` via `claudeMdExcludes`); add a `paths:` frontmatter filter to any `.claude/rules/` file that currently loads unconditionally
   but only applies to specific file types; prune stale memory; switch off or scope the MCP servers the work does not use (the largest
   single lever, and keep tool search on so unused servers' schemas stay deferred); consolidate redundant skills.
