...
- **Operating** an MCP surface (not auditing server code) — how MCP connectors added through claude.ai are governed across the web UI, org admin console, and Claude Code's `managed-mcp.json` allow/deny layer — is in [claude-ai-connector-control.md](references/claude-ai-connector-control.md). Out of this skill's code-audit scope, but the natural home for the reference.
