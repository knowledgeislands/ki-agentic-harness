...
for (const d of ['config', 'mcp-server', 'tools', 'main', 'utils']) {
  isDir('src', d)
    ? add('PASS', 'layout', `src/${d}/ present`, STD, `src/${d}`)
    : add('FAIL', 'layout', `src/${d}/ missing`, STD, `src/${d}`)
}
const hasCli = isDir('src', 'cli')
if (hasCli) {
  for (const f of ['cli.ts', 'index.ts']) {
    has('src', 'cli', f)
      ? add('PASS', 'layout', `src/cli/${f} present`, STD, `src/cli/${f}`)
      : add('FAIL', 'layout', `src/cli/ exists but src/cli/${f} missing`, STD, `src/cli/${f}`)
  }
}
...
try {
  ...
  add('FAIL', 'package', 'package.json missing or unparseable', STD, 'package.json')
}
...
if (scripts['ki:test:smoke'] && has('.github', 'workflows', 'ci.yml')) {
  read('.github', 'workflows', 'ci.yml').includes('bun run ki:test:smoke')
    ? add('PASS', 'ci', 'ci.yml runs ki:test:smoke (MCP delta, after the common gate)', STD, '.github/workflows/ci.yml')
    : add(
        'FAIL',
        'ci',
...
Object.values(bin).includes('dist/mcp-server/index.js')
  ? add('PASS', 'package', 'bin → dist/mcp-server/index.js', STD, 'package.json')
  : add('FAIL', 'package', 'bin must map to dist/mcp-server/index.js', STD, 'package.json')

const exp = (pkg.exports ?? {}) as Record<string, unknown>
for (const k of ['.', './config', './package.json']) {
  exp[k] !== undefined
    ? add('PASS', 'package', `exports has "${k}"`, STD, 'package.json')
    : add('FAIL', 'package', `exports missing "${k}"`, STD, 'package.json')
}
...
for (const k of ['ki:server:mcp:dev', 'ki:server:mcp:inspect', 'ki:server:mcp:start']) {
  scripts[k]
    ? add('PASS', 'scripts', `${k} present`, STD, 'package.json')
    : add('WARN', 'scripts', `MCP script "${k}" missing`, STD, 'package.json')
}
// ki:generate:client — the mcporter typed-client codegen, required for every MCP.
scripts['ki:generate:client']
  ? add('PASS', 'scripts', 'ki:generate:client present (mcporter typed-client codegen)', STD, 'package.json')
  : add('FAIL', 'scripts', 'MCP script "ki:generate:client" missing — the mcporter typed-client codegen', STD, 'package.json')
...
if (isDir('src', 'auth-server')) {
  for (const k of ['ki:server:auth:dev', 'ki:server:auth:start']) {
    scripts[k]
      ? add('PASS', 'scripts', `${k} present (auth-server delta)`, STD, 'package.json')
      : add('FAIL', 'scripts', `src/auth-server/ present but "${k}" missing (auth-server delta)`, STD, 'package.json')
  }
}
// Record/replay integration harness: ki:test:record / ki:test:replay travel as a pair.
{
  ...
  if (rec !== rep)
    add(
      'WARN',
      'scripts',
      'ki:test:record and ki:test:replay must be defined together (mcporter record/replay harness)',
      STD,
      'package.json'
    )
  else if (rec) add('PASS', 'scripts', 'ki:test:record + ki:test:replay present (integration harness)', STD, 'package.json')
}

// ── shared utils helpers ──────────────────────────────────────────────────────
for (const f of ['access-level.ts', 'annotations.ts', 'audit-log.ts']) {
  has('src', 'utils', f)
    ? add('PASS', 'utils', `utils/${f} present`, STD, `src/utils/${f}`)
    : add('FAIL', 'utils', `shared utils/${f} missing`, STD, `src/utils/${f}`)
}
...
if (cfg) {
  ;/export\s+(async\s+)?function\s+loadConfig|export\s+const\s+loadConfig/.test(cfg)
    ? add('PASS', 'config', 'config exports loadConfig', STD, 'src/config/index.ts')
    : add('FAIL', 'config', 'config/index.ts does not export loadConfig', STD, 'src/config/index.ts')
  ...
  for (const sym of ['ACCESS_LEVELS', 'ACCESS_LEVEL_RANK', 'AuditLogMode']) {
    cfg.includes(sym)
      ? add('PASS', 'config', `config references ${sym}`, STD, 'src/config/index.ts')
      : add('WARN', 'config', `config missing ${sym}`, STD, 'src/config/index.ts')
  }
}
...
if (vitestFile) {
  ...
  for (const [label, re] of coverageExcludes) {
    re.test(vc)
      ? add('PASS', 'vitest', `coverage excludes ${label}`, STD, vitestFile)
      : add('WARN', 'vitest', `coverage should exclude ${label}`, STD, vitestFile)
  }
}
...
if (toolNames.length) {
  ...
  add('PASS', 'tools', `registered tools (${toolNames.length}): ${toolNames.sort().join(', ')}`, STD)
  bad.length
    ? add('WARN', 'tools', `names not matching <app>_<resource>_<action> (or _<action> for metadata): ${bad.join(', ')}`, STD)
    : add('PASS', 'tools', 'all tool names look like <app>_<resource>_<action>', STD)
} else {
  add('WARN', 'tools', 'no registerTool(...) calls found — verify tool registration', STD)
}
...
if (isDir('src', 'tools')) {
  ...
  else if (usesStructured) add('PASS', 'tools', 'structuredContent paired with a declared outputSchema', STD)
