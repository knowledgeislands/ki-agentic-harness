...
- **CI — the smoke delta.** The common CI shape (`jdx/mise-action` + `bun run ki:lint:check` / `ki:lint:types` / `ki:lint:md:check` / `test:coverage`) is `ki-engineering`'s, asserted by `audit.ts`. An MCP repo with a smoke harness **appends `bun run ki:test:smoke`** to `.github/workflows/ci.yml` after those steps — the MCP delta on the CI shape, asserted here by `audit.ts`.
