#!/usr/bin/env bun
/**
 * Mechanical CONFORM for the ki-kb standard — fixes the subset of audit-kb.ts's
 ...
 * (conform-repo.ts, conform-drs.ts) — `bun conform-kb.ts .` / `ki:kb:conform`.
 ...
 * with audit-kb.ts (same source of truth, copied rather than imported so each
 * script stays valid standalone per the composition-only rule).
 *
 *   bun scripts/conform-kb.ts [path]   # default: cwd
 *   --dry-run                           # print the plan, mutate nothing
 *
 * Fixes:
 *   - Config marker: when `.ki-config.toml` has no `[ki-kb]` table at all, appends
 *     the bare opt-in marker (audit-kb.ts's `--init` template) — never overwrites
...
// ── kept in lockstep with audit-kb.ts ──
...
// Same opt-in marker audit-kb.ts's `--init` emits.
...
// Same minimal parser as audit-kb.ts — a full TOML parser is unnecessary for
...
async function main() {
  ...
  console.log(
    `\n${paint(C.dim, 'mechanical layer applied — re-run `bun scripts/audit-kb.ts .` (or `ki:kb:audit`) to confirm findings clear.')}`
  )
}
