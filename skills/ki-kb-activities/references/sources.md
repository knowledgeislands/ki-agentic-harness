...
| --- | --- | --- |
| arcadia-principal `Admin/Operations/Activities/` | Live base sampled for realization types in use — currently PRE-ADOPTION: legacy activity-group notes, non-conformant (no realization field, free-text status) | 2026-07-04 |
| `ki-kb-base` SKILL.md | Zone model and Admin/ subdivision that hosts activities | 2026-07-04 |
...
- **Reference-implementation gap.** The skill has no live, conformant reference base. Either conform arcadia-principal's Activities notes (add `realization`, switch status to the enum, add a `[ki-kb-activities]` table) so the citation is honest, or reclassify the citation as a pre-adoption sample. Decide direction before next REFRESH.
...
- **Legacy skill-name rename.** arcadia-principal still uses `knowledgeislands-*` config names; when it adopts `ki-kb-activities` it should use the `ki-*` name. Base-migration concern, tracked here only because it blocks the reference base from becoming conformant.
