...
This file is the **normative, quotable** standard. The checkable items live in [the rubric](rubric.md); the mechanical checks are in [`../scripts/audit.ts`](../scripts/audit.ts); the meta-standard for how this (and any) standard is defined and enforced is [the enforcement framework](enforcement-framework.md).
