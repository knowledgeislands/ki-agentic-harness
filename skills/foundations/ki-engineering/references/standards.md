...
This file is the **normative, quotable** standard. The checkable items live in [the rubric](rubric.md); the mechanical checks are in [`../scripts/audit.ts`](../scripts/audit.ts); the model for turning this (or any) standard into an executable rubric is `ki-skills`' [rubric-authoring guide](../../../general-governance/ki-skills/references/rubric-authoring.md).
