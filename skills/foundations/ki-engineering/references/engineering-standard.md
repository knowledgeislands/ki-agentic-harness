...
Where the repo has CI (`.github/workflows/ci.yml`), it is a single `build` job on `push` to `main` and `pull_request`, running the common gate **in order**: `jdx/mise-action` (installs the toolchain from `mise.toml`, pinning **no** version itself — no `bun-version:` / `node-version:`, which would bypass `mise.toml` and is drift) → `bun install --frozen-lockfile` → **`bun run ki:verify`**. The single `ki:verify` step _is_ the gate — it composes `ki:lint:check` → `ki:lint:types` → **`ki:lint:md:check`** (+ `build`, then either the runner-neutral `test` or Vitest-profile `test:coverage` where those capabilities are present), in order (§2). The Markdown gate is load-bearing: `ki:lint:md` self-heals locally with `--write`, so only its `--check` twin (inside `ki:verify`) stops prose-wrap drift reaching `main`. A `ki:test:smoke` step that follows in an MCP repo is that artifact's **delta**, owned by `ki-mcp` and asserted by `audit.ts` — not part of this common shape.
...
// repos with a compiled build append " && bun run build"; repos with tests append " && bun run test",
// using "test:coverage" instead only under the Vitest profile.
```
...
- **`ki:verify`** (read-only) is the local mirror of the CI gate (§1 _CI workflow_): `ki:lint:check` / `ki:lint:types` / `ki:lint:md:check` / **`ki:knip`** (the dependency + dead-code gate) (+ `build`, then runner-neutral `test` or Vitest-profile `test:coverage`), in order, mutating nothing. CI runs `bun run ki:verify` as its single gate step.
