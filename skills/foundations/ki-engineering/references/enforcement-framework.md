...
- **EDUCATE** — scaffold a new artifact to the standard, or bring an off-standard one onto it from scratch. Its mechanical half is a per-skill **`scripts/educate.ts`** — the EDUCATE counterpart to AUDIT's `audit-*.ts` — that detects the existing shape, applies safe defaults, declares and triggers the skills this one **`implies:`** (the frontmatter graph, rendered by `ki:skills:graph`), and satisfies the self-sufficiency contract: it runs from the remote source with **no skill installed locally**, vendoring script copies (SCRIPT-7) and HELP snapshots into `.ki-meta/` and writing the four `.ki-meta/bin/{ki-audit,ki-conform,ki-educate,ki-help}` wrappers, so the target governs itself afterwards with zero skills installed; it does **not** write `package.json` (that is `ki-engineering`'s own concern, as sugar over the bins). Under an LLM it also applies judgment (choosing among shapes, filling prose); as pure CLI it takes the safe defaults and **prints the judgment checks it skipped**. `ki-bootstrap` starts the chain (always pulls `ki-repo`); re-running the idempotent chain is the single way to bring a target up to date.
