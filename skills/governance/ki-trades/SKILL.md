...
6. **Observation-led release** — itemized knowledge or work may use `unattended` or `receipt`, both requiring evidenced receipt before release; unattended requests no response and grants no execution authority. Work may instead use `decision` or `completion`. Completion fails closed until a selected adapter supplies owner-valid canonical evidence. The receiver may prune only after eligible sender release is observable.
