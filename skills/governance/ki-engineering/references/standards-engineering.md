...
`.husky/commit-msg` begins with `bunx commitlint --edit "$1" || exit 1`. The wholly governed `commitlint.config.mjs` extends `@commitlint/config-conventional` and restricts types to `chore`, `docs`, `feat`, `fix`, `refactor`, and `test`, with lowercase kebab-case scopes and non-empty subjects without a terminal full stop. `ki-git` owns that vocabulary and the judgment that a message accurately describes one completed unit; engineering owns only the package-backed binding and deterministic shape.
