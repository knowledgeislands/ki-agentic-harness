...
- **A monorepo detects its own change, because Bun will not do it for you.** Bun workspaces resolve dependencies; they supply no task graph, no input hashing, and no notion of a target being up to date. A root script that chains `bun run --cwd <workspace> build` across every package is a fixed sequence, so every gate run, every `dev` start, and every CI job rebuilds all of it — including the seven workspaces nobody touched. The default remedy is **local, dependency-free change detection**, not an orchestrator: derive the build order from each workspace's own manifest rather than a hand-maintained list, fingerprint the inputs its build configuration actually declares, fold each dependency's fingerprint into its dependents' so a change reaches everything downstream, and skip a workspace whose fingerprint matches what was recorded for its current output. Keep a `--force` escape so a build can always be proved from nothing. Cheaper still, and orthogonal: point tests and type-checks at each workspace's own source (§4 `paths`, the test runner's aliases) so the build leaves the critical path of the stages that run most often, and give the type-check `--incremental` build information per project so an untouched project reports in well under a second.
- **Four rules make a local cache safe, and skipping any one of them makes it a liar.** Fingerprint file _contents_ and pathnames, never modification times — a checkout, a `touch`, or a formatter pass moves an mtime without changing a byte that is compiled. Treat a recorded fingerprint as a claim rather than evidence: rebuild unless the output is also present, because deleting `dist` touches no input. Clear the record _before_ building rather than writing it after failing, so an interrupted run leaves nothing behind that asserts output it never produced. And fold the lockfile into every workspace's fingerprint, because a different compiler emits different declarations from identical sources and an upgrade must invalidate work nobody edited. The input set must match what the build configuration includes and excludes: one that is nearly right skips exactly the workspace that changed, which is the single failure this machinery exists to prevent, so guard it with a test that makes a real edit and asserts the workspaces downstream of it stop matching — cover the exclusions too, or a test file will rebuild half the repository forever.
- **Cache the intermediate builds, not the published artifact.** A website or deployable's bundle is the thing the gate exists to prove, its input set is large and implicit (content, public assets, routes, every dependency's `dist`, the bundler configuration and its plugins), and skipping it only ever pays on a run where nothing at all changed — edit a package and its inputs have moved; edit the site and its own sources have. An input list that is nearly right there publishes a stale site. Measure before caching anything else: generators and codegen checks are often already in the tens of milliseconds, where there is nothing to detect.
- **Reach for an orchestrator (Turborepo, Nx) when the count of _steps_ earns one, not the count of packages.** The threshold is the point where the local runner stops being a script and becomes its own maintained tool: many distinct build steps per workspace with real ordering between them, a pipeline whose stages differ per workspace, or a shared remote cache paying off across machines and CI. Turborepo is package-manager agnostic and sits over Bun workspaces without replacing them, so this is a genuine upgrade path rather than a migration. Below that threshold it trades a hundred-line script for a dependency, a configuration language, and a second cache to reason about when a result looks wrong. Record which side of the line the repository sits on and why, so the next contributor inherits the decision rather than the symptom.
