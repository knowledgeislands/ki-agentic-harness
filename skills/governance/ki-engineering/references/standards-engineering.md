...
- **The mechanical audit proves only the task graph it can see.** `TURBO-1` warns when a workspace repository has no readable, non-empty `turbo.json`; `TURBO-2` checks the required workspace lifecycle scripts and their configured tasks; `TURBO-3` checks explicit remote-cache intent, the local-cache ignore, root dependency boundaries, and `$TURBO_DEFAULT$` on mechanically identifiable deployables. Whether every task input covers every file the command reads remains a mutation test: change each declared input with random content and prove the task reruns. An inspection must not report that semantic property as mechanically verified.
