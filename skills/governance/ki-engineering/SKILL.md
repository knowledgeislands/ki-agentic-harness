...
- **Monorepo builds detect their own change** — Bun workspaces give no task graph and no input hashing, so the default is a local, dependency-free fingerprint over each workspace's declared build inputs, folding in its dependencies' fingerprints, with a `--force` escape. Fingerprint contents rather than mtimes, never trust a record without its output, and cache the intermediate builds rather than the published bundle. Reach for Turborepo or Nx when the number of ordered build _steps_ — or a shared remote cache across machines and CI — earns the dependency.
