 ...
 * audit-engineering.ts's findings that are unambiguous and reversible, leaving
 ...
 * (conform-repo.ts, conform-drs.ts) — `bun conform-engineering.ts .` /
 ...
 * audit-engineering.ts rather than imported, so each script stays valid
 * standalone per the composition-only rule.
 *
 *   bun scripts/conform-engineering.ts [path]   # default: cwd
 *   --dry-run                                    # print the plan, mutate nothing
 *
 * Fixes:
 *   - package.json: `type`, `packageManager`, `engines.node`, the exact
 *     `ki:lint:*` / `ki:deps:*` script families (CANON), `clean`, `prepare`,
 ...
 *     audit-engineering.ts checks against. Never overwrites an existing file
 ...
 *     table is missing (mirrors conform-repo.ts's own config-marker append).
 *
 ...
 *   - Monorepo-specific checks (workspaces-aware ki:lint:types, per-workspace
 *     vitest scoping) — repo-shape-specific, not a single mechanical fix.
...
// ── kept in lockstep with audit-engineering.ts ──
...
function log(kind: 'fix' | 'write' | 'append' | 'skip', label: string) {
...
// ── package.json: required script families (exact-match, overwrite) ──
console.log(`\n${paint(C.cyan, 'package.json — ki:lint:* / ki:deps:* script families')}`)
{
  ...
  // ki:conform / ki:verify composed entrypoints — only fix when absent; an existing
  // but incomplete composition is judgment (which capability tails to append).
  if (!scripts['ki:conform']) {
    scripts['ki:conform'] =
      'bun run ki:deps:refresh && bun run ki:lint:package && bun run ki:lint:format && bun run ki:lint:fix && bun run ki:lint:md'
    log('fix', 'ki:conform: missing → added (base composition; append build/test tails by hand if this repo has them)')
    any = true
  }
  if (!scripts['ki:verify']) {
    scripts['ki:verify'] = 'bun run ki:lint:check && bun run ki:lint:types && bun run ki:lint:md:check && bun run ki:knip'
    log('fix', 'ki:verify: missing → added (base composition; append build/test:coverage tails by hand if this repo has them)')
    any = true
  }
  if (any) {
    pkg.scripts = scripts
...
manualTodos.push(
  'CI workflow content [ci] — .github/workflows/ci.yml must be authored/edited by hand (mise-action install, no hardcoded version, "bun run ki:verify" gate)'
)
manualTodos.push(
  'monorepo-specific checks [scripts/tests, §0] — workspaces-aware ki:lint:types aggregation and per-workspace vitest scoping need a human call on repo shape'
)
manualTodos.push(
...
console.log(
  `\n${paint(C.dim, 'mechanical layer applied — re-run `bun scripts/audit-engineering.ts .` (or `ki:engineering:audit`) to confirm findings clear.')}`
)
