...
- [ ] [M] WARN — the `ki:deps:*` family + `ki:knip` are present and exact-match their canonical knip-based values (standard §2): `ki:deps:check` (knip, deps-scoped), `ki:deps:fix` (same `+ --fix`), `ki:deps:update` (`bun update --force`), `ki:knip` (full knip).
