...
- **INIT** — scaffold a new artifact to the standard, or bring an off-standard one onto it from scratch. Its mechanical half is a per-skill **`scripts/init.ts`** — the INIT counterpart to AUDIT's `audit-*.ts` — that detects the existing shape, applies safe defaults, declares and triggers the skills this one **`implies:`** (the frontmatter graph, rendered by `ki:skills:graph`), and satisfies the self-sufficiency contract: it runs from the remote source with **no skill installed locally**, vendoring script copies (SCRIPT-7) and installing the skill's `ki:<suffix>:{init,audit,conform}` package.json keys plus the repo-wide `ki:audit`/`ki:conform`/`ki:init` aggregates, so the target governs itself afterwards with zero skills installed. Under an LLM it also applies judgment (choosing among shapes, filling prose); as pure CLI it takes the safe defaults and **prints the judgment checks it skipped**. `ki-bootstrap` starts the chain (always pulls `ki-repo`); the new/legacy/tracking aggressiveness flags are the same chain at different strengths (new = INIT; legacy = INIT + full CONFORM; tracking = AUDIT + mechanical CONFORM).
