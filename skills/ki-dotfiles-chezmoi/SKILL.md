...
This is a **standard, tool-agnostic Process skill** — it hard-codes no specific dotfiles content and assumes no particular set of managed applications. Install it in any chezmoi source repo. How it sits alongside the other skills in this repository is documented once in the `ki-agentic-harness` `README.md`, not repeated here.
...
- **App-mutated config handling** — Pattern A (surgical patch) vs Pattern B (full template + reverse-merge), with a decision rule.
...
2. Apply the **judgment** (`[J]`) criteria the ADVISORY findings name — Pattern A/B correctness for a given app config, CLAUDE.md Layer 1/2 placement quality, `.chezmoiignore` negation intent, whether audit-reporting etiquette was actually followed.
...
chezmoi's own documented behavior (naming semantics, `.chezmoiignore`, `run_onchange_` scripts, health-check commands) is the authoritative layer and moves slowly; the house-convention layer on top of it (shell-loader pattern, the two app-mutated-config patterns, the templating pattern) is this skill's own judgment and should be re-anchored as more repos are audited against it, not just against chezmoi's docs. Run on the declared cadence (see [the sources list](references/sources.md)), or when asked "is the chezmoi standard current".

1. **Read [the source list](references/sources.md)** — each tracked source with its `last reviewed` date.
2. **Re-fetch each** (WebFetch; fall back to WebSearch if a host is blocked or returns non-200) and diff against [the standard](references/dotfiles-standard.md): a changed chezmoi naming/templating behavior, a new `run_onchange_` capability, a pattern this standard hasn't captured yet.
