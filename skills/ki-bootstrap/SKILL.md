...
1. **INIT — mechanical self-sufficiency (the chain).** Bring a target repo under governance so it self-governs with `./.ki-meta/bin/ki-audit` and **zero skills installed** — and with no `package.json` of its own, ever touched: for every skill in the resolved set, vendor _copies_ of its declared mechanical unit plus a rendered HELP snapshot into the repo's `.ki-meta/`, and write the four `.ki-meta/bin/{ki-audit, ki-conform, ki-init, ki-help}` wrappers over the `.ki-meta/bin/aggregate.ts` runner. Bootstrap writes **no** `package.json` keys — those are `ki-engineering`'s to wire, as sugar over these bins. This is how any repo bootstraps and re-bootstraps up to date — run from the remote source, no local install, idempotent. See **Mode INIT** and [ADR-KI-HARNESS-007](../../docs/decisions/ADR-KI-HARNESS-007-bootstrapping-and-self-sufficiency.md).
