...
- [ ] [M] FAIL — `TOOL-BIN`: `bin/` exists and holds ≥1 file. Absent ⇒ FAIL (no tool).
- [ ] [M] FAIL — `TOOL-EXEC`: every `bin/<file>` carries the executable bit (`statSync(mode) & 0o111`). Git tracks the exec bit; a bin file without it breaks the installer and formula.
...
- [ ] [M] WARN — `TOOL-INSTALL`: `install.sh` is present at the repo root and executable (the `curl | bash` contract).
- [ ] [J] WARN — `install.sh` is POSIX-ish, honours env overrides (target dir + version/ref), verifies the download, and is idempotent.
- [ ] [M] WARN — `TOOL-VERSION`: the primary bin file contains `--version` handling (grep). ADVISORY when the file can't be read.
...
- [ ] [M] WARN — `TOOL-CHANGELOG`: `CHANGELOG.md` is present. (README / LICENSE are `ki-repo`'s — not checked here.)
...
- [ ] [M] WARN — `TOOL-CI`: at least one `.github/workflows/*.yml` is present.
...
- [ ] [M] WARN — `TOOL-TESTS`: a `tests/` directory is present (the executable test suite).
- [ ] [M] WARN — `SHELL-LINT` (capability): **if** the primary bin has a `bash`/`sh` shebang, a CI workflow references `shellcheck`.
- [ ] [M] WARN — `SHELL-TEST` (capability): **if** shell, `tests/` holds a `*.bats` file **and** a CI workflow references `bats`.
- [ ] [M] INFO — `LANG-DEFER`: **if** a `package.json` is present, the repo is a TS/Bun tool — it defers lint/test to `ki-engineering` and MUST also declare `[ki-engineering]`.
...
- [ ] [M] WARN — `CONFIG`: a `[ki-tools]` table is present in `.ki-config.toml` (the opt-in marker). Missing file or table ⇒ WARN.
- [ ] [M] WARN — `CONFIG`: validate-down — any key inside `[ki-tools]` is unknown today and WARNs.

## Releases

- [ ] [J] ADVISORY — `RELEASE`: releases are `vX.Y.Z` git tags, each with a GitHub release; the marker, tag, and CHANGELOG top entry agree. Not checkable from a path — the checker emits an ADVISORY; verify tags/releases by hand (`git tag`, `gh release list`).
