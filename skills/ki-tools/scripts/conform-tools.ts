 ...
 * audit-tools.ts's findings that are unambiguous and reversible, leaving
 ...
 * conform shape (conform-repo.ts, conform-drs.ts) — `bun conform-tools.ts .`
 * / `ki:tools:conform`. bin/ discovery and the [ki-tools] marker default are
 * kept in lockstep with audit-tools.ts (same source of truth, copied rather
 * than imported so each script stays valid standalone per the
 * composition-only rule).
 *
 *   bun scripts/conform-tools.ts [path]   # default: cwd
...
// ── kept in lockstep with audit-tools.ts ──
...
// audit-tools.ts's parseKiTools (returns null when the table is absent).
...
async function main() {
  ...
  console.log(
    `\n${paint(C.dim, 'mechanical layer applied — re-run `bun scripts/audit-tools.ts` (or `ki:tools:audit`) to confirm findings clear.')}`
  )
}
