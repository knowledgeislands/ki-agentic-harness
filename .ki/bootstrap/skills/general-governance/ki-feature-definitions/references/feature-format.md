...
The quotable standard behind [`../SKILL.md`](../SKILL.md). It defines how a Feature Definition corpus is laid out, how a single requirement is written, and the rules the checker ([`../scripts/audit.ts`](../scripts/audit.ts)) enforces. The line-by-line criteria are in [rubric.md](rubric.md); worked examples are in [exemplars.md](exemplars.md).
