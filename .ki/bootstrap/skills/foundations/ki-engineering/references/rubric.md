...
- **SCR-4 [M] — derived checker entrypoints** — Every checker payload in `.ki/bootstrap/checkers/<skill>/` is reachable through derived `ki:<suffix>:audit` and `ki:<suffix>:conform` keys. (standards.md)
