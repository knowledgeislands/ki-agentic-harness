...
try {
  ...
  check(
    ...
    finding?.message?.includes('.ki/bin/ki-educate') === true
  )
} finally {
...

{
  ...
  check(
    'EDUCATE help explains local per-skill refresh',
    result.out.includes('Usage: bun scripts/educate.ts') && result.out.includes('Refresh only ki-repo')
  )
}
