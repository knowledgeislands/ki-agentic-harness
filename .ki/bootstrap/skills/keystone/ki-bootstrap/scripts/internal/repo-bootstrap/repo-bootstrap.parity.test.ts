...
try {
  ...
  const checkers = join(fixture, '.ki', 'bootstrap', 'checkers')
  ...
  chmodSync(join(fixture, '.ki', 'bin', 'ki-audit'), 0o755)
  chmodSync(join(fixture, '.ki', 'bin', 'ki-conform'), 0o755)

  const nested = join(fixture, 'nested', 'cwd')
  mkdirSync(nested, { recursive: true })
  const env = { KI_EXPECT_ROOT: fixture, KI_FIXTURE_FAIL: '0' }
  const auditWrapper = join(fixture, '.ki', 'bin', 'ki-audit')
  const conformWrapper = join(fixture, '.ki', 'bin', 'ki-conform')
  const aggregate = join(fixture, '.ki', 'bin', 'aggregate.ts')
  ...
  const scopedPass = run(
    'bun',
    [join(fixture, '.ki', 'bin', 'aggregate.ts'), 'audit', '--skill', 'ki-fixture', '--reporter-levels=all'],
    fixture,
    env
  )
  check(
  ...
  const unknownScope = run('bun', [join(fixture, '.ki', 'bin', 'aggregate.ts'), 'audit', '--skill', 'ki-missing'], nested, env)
  check(
