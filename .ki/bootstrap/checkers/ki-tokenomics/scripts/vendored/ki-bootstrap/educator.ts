...
export const createSkillEducationPlan = (request: SkillEducationRequest): SkillEducationPlan => {
  ...
  const checker = join(target, '.ki', 'bootstrap', 'checkers', request.skill)
  const educator = join(target, '.ki', 'bootstrap', 'educators', request.skill)
...
export const publishSkillEducation = (plan: SkillEducationPlan): void => {
  ...
  const ki = join(plan.target, '.ki')
  ensurePhysicalDirectory(ki, 'governance directory')
  const bootstrap = join(ki, 'bootstrap')
  ensurePhysicalDirectory(bootstrap, 'bootstrap directory')
  const checkers = join(bootstrap, 'checkers')
  const educators = join(bootstrap, 'educators')
  ensurePhysicalDirectory(checkers, 'checker directory')
  ensurePhysicalDirectory(educators, 'educator directory')

  const staging = mkdtempSync(join(ki, `.educate-${plan.skill}-`))
  const candidateRoot = join(staging, 'candidate')
  const backupRoot = join(staging, 'backup')
  const destinations = [
    {
      candidate: join(candidateRoot, '.ki', 'bootstrap', 'checkers', plan.skill),
      destination: join(checkers, plan.skill),
      backup: join(backupRoot, 'checker')
    },
    {
      candidate: join(candidateRoot, '.ki', 'bootstrap', 'educators', plan.skill),
...
Refresh only ${skill} under the target repository's .ki/bootstrap/checkers and
.ki/bootstrap/educators directories. Aggregate runners remain owned by ki-bootstrap.
