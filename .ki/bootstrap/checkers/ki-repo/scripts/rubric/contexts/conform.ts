/** Safe repository mutation and outcome collection for the ki-repo CONFORM rubric. */
/**
...
import type { RepoEvidenceFinding, RepoEvidenceLevel } from './audit.ts'
...
export type RepoConformCollection = {
  target: string
  findings: readonly RepoEvidenceFinding[]
}

class ConformComplete extends Error {
  constructor(readonly collection: RepoConformCollection) {
    super('ki-repo conform evidence collection complete')
  }
}

export const collectConformFindings = (argv: readonly string[]): RepoConformCollection => {
  try {
    ...
    type LegacyConformLevel = RepoEvidenceLevel
    const findings: RepoEvidenceFinding[] = []
    const rec = (level: LegacyConformLevel, code: string, message: string, _ref?: string, file?: string): void =>
      void findings.push({
        level,
        code,
        message,
    ...
    function finishConform(target: string): never {
      throw new ConformComplete({ target, findings })
    }
    ...
    // each fine check with its own code. dry-run records the planned action as FIXED.
    ...
    // still records FIXED on write / FAIL on error, dry-run included).
    ...
    // file = the scaffolded path: written → FIXED, already present → PASS (never overwritten).
    ...
    function scaffold(name: string, area: string, snapshot: LeafSnapshot): void {
      ...
      rec('FIXED', area, `${name} scaffolded (was missing)`, STD, name)
    ...
    if (plannedConfig.addAuthoring) {
      rec('FIXED', 'FILES-3', `${KI_CONFIG} [ki-authoring] marker appended`, STD, KI_CONFIG)
    ...
    if (!m) {
      rec('NOT_APPLICABLE', 'ACCESS-1', `origin is not on github.com (${origin ?? 'no origin'}) — GitHub settings skipped`, STD)
    ...
    try {
      ...
      rec('NOT_APPLICABLE', 'ACCESS-1', `could not read repos/${nwo} via gh — GitHub settings skipped (network or authentication)`, STD)
      finishConform(target)
    }
    ...
    if (enforced('branch-protection') && branchProtectionOn) {
      ...
      if (dryRun) {
        ...
        rec('FIXED', 'BP-1', `would set branch protection on main (opted in via [${CHECKS_SECTION}])`, STD)
      } else {
        try {
          ...
          rec('FIXED', 'BP-1', `branch protection on main (opted in via [${CHECKS_SECTION}])`, STD)
        } catch (e) {
          const m = String((e as Error).message ?? e).split('\n')[0]
      ...
      rec('FIXED', 'BP-1', 'would strip any leftover branch protection (default: off)', STD)
    } else {
      try {
        ...
        rec('FIXED', 'BP-1', 'stripped leftover branch protection (default: off)', STD)
      } catch (e) {
    ...
    ghIfNeeded(
      autoSecurityFixesOn,
      ['api', '-X', 'PUT', `repos/${nwo}/automated-security-fixes`],
      'DEP-1',
    ...
    if (isPublic && (enforced('secret-scanning') || enforced('push-protection'))) {
      ...
      if (allConformant) {
        ...
        rec('FIXED', 'SEC-1', `would set secret scanning / push protection (covers: ${covers})`, STD)
      } else {
        try {
          ...
          rec('FIXED', 'SEC-1', `secret scanning / push protection (covers: ${covers})`, STD)
        } catch (e) {
          const m = String((e as Error).message ?? e).split('\n')[0]
    ...
    if (error instanceof ConformComplete) return error.collection
    throw error
  }
}
