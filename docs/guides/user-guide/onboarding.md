...
The stable public route is `/harness/bootstrap`; use the raw GitHub form above until that website route is published.
...
```bash
mgit -B sh -c 'curl -fsSL https://raw.githubusercontent.com/knowledgeislands/ki-agentic-harness/main/skills/keystone/ki-bootstrap/scripts/repo-bootstrap.sh | sh'
```
...
```bash
bun "$KI_HARNESS/skills/keystone/ki-bootstrap/scripts/lib/repo-bootstrap.ts" "$TARGET"
```
