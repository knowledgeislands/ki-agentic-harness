...
Each thematic roadmap item has a stable qualified locator, `<theme>/<item-slug>`. Plan ids remain globally unique across themes, and dependencies use those global ids. An item has exactly one authoritative home. Work requiring an executable plan expands into the thematic profile before the plan is created.
...
- Global plan ids preserve simple cross-theme dependencies, while qualified item locators remove title-only ambiguity.
