...
- **Native in-process resolution.** `ki` resolves the selected repository's declared skills and explicit dependencies from the verified active collection, orders their registered operations, and reports results through one shared model. It neither spawns a child process nor imports a vendored `govern.ts`; the `lint` verb is retired.
