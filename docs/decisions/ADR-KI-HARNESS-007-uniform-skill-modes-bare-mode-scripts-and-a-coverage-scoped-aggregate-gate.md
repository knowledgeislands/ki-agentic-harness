...
Every **governance** skill exposes exactly four universal acting modes through one uniform native-operation registration — no per-skill override. It also exposes HELP as a required non-acting entry point, not as a fifth mode.

- **One native operation registration.** Every governance skill registers compatible operation metadata and in-process implementations for its mechanical modes. `ki` hosts the public `audit` and `conform` operations; REFRESH remains collection-authoring work, and the HELP entry point remains skill documentation rather than a generated repository runner.
...
- A selected audit NA-skips when the repository lacks the signal it governs. Coverage-scoping excludes undeclared skills; it does not replace each audit's own applicability boundary. The aggregate gate is complete for the repository's selected coverage.
