# ADR-KI-HARNESS-009: Subagent isolation for multi-skill invocation
...
agent context — one skill at a time in dependency order (ADR-KI-HARNESS-007). This approach achieved its primary goal of bounding context
(load one skill, release it before loading the next), but it serialised work that is largely independent, meaning a full harness audit took
as long as the sum of all concern durations.
...
3. **Synthesise in the main agent** — collect all subagent results, rank findings with dependency-order priority (foundations first, per
   ADR-KI-HARNESS-007), and report across concerns.
...
- The dependency order from ADR-KI-HARNESS-007 becomes synthesis ranking priority, not execution order.
...
- ADR-KI-HARNESS-007 — the serial dependency-order walk this supersedes.
