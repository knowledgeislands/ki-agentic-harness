...
- **Install the harness once for a user.** `https://knowledgeislands.info/harness/install` serves the `scripts/user-install.sh` entry point. It installs regular-file copies of the global core skills (`ki-bootstrap`, `ki-delegate`, `ki-next`, `ki-plan`, and `ki-recap`) for the selected runtime. When Claude Code is selected, it also installs the durable Claude hook payload. It owns only its managed user-space payload directories; it never creates a development symlink or writes Claude settings.
