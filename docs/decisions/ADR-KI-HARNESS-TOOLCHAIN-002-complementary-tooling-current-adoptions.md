...
**mcporter** — adopted in two roles. As the **MCP proxy daemon**, KI-owned local stdio servers sit behind mcporter's keep-alive daemon, exposed as a single `ki-mcporter` URL entry (`http://localhost:3333/mcp`) in `~/.claude.json`, with tools namespaced `server__tool`; this is the subject of ADR-KI-HARNESS-TOOLCHAIN-003, later in the reading order. As a **typed client**, mcporter generates each `mcp-*` repo's typed client per-repo via that repo's `ki:generate:client` script (owned by `ki-repo-mcp`'s audit/conform). Installed via Homebrew (`brew install steipete/tap/mcporter`); chezmoi deploys the config and LaunchAgents.
