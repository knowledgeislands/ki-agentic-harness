...
§ Codex subagents are TOML under `~/.codex/agents/` (`name`/`description`/`developer_instructions`), a different shape from Claude Code's Markdown+YAML. The generated agent publisher reports-and-skips Codex pending the generator; the format is spiked, not built.
