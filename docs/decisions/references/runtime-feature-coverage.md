...
**Pure KI invention (already runtime-agnostic — the model to imitate).** The paired skills ki-plans and ki-plan define a KI-invented `docs/plans/*.md` file format plus a slash-command lifecycle with **zero** native-tool dependency: no runtime plan primitive is involved, so the surface is identical on any runtime that can read a file and route a slash command. The governance skills are the same shape — plain `SKILL.md` plus Markdown plus a checker, depending only on the file layout.

**Two different "plan" concepts, easy to conflate.** Claude Code's interactive **Plan Mode** is a runtime tool pair (`EnterPlanMode` / `ExitPlanMode`) that the plan-stamp / plan-sync hooks latch onto — runtime-native and fragile. The ki-plans / ki-plan **plan** is a KI-authored governance artefact (`docs/plans/*.md`) with a slash-command lifecycle — a file format the harness owns outright, independent of any runtime plan tool. Same word, entirely different mechanisms and entirely different portability profiles; do not treat a claim about one as evidence about the other.
