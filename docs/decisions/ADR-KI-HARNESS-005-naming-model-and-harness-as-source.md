...
- **`ki-agentic-harness`** is the framework's general tooling — the skills, agents, MCP wrappers, and evals any island adopts. It is framework-level (scope `KI-HARNESS`), not Arcadia-territory-scoped.
...
- The four-part bundle stays the single source of truth. Distribution to any surface is a projection performed by `ki-binding`, not a fork or a rewrite; nothing "becomes" a plugin.
