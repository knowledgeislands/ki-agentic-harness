...
- **Every environment has a mechanical prompt.** Whatever the repo type — engineering or not, `package.json` or not — a governed repository has a model-free invocation surface through the installed `ki` tool. `ki repo audit` and `ki repo conform` resolve that repository's declared coverage from the verified active skill collection. This keeps the mechanical baseline universal without copying an executor into each repository.
...
- This is the precondition for repository self-sufficiency: mechanical operations run without an LLM, but a clean clone first acquires the verified active skill collection that supplies its declared operations.
- The concrete surface of the mechanical half is a skill's registered native operation in the verified active collection. Repository declarations select those operations and their explicit dependencies; the native `ki` host orders and reports them. A package-script alias may be a local convenience, but it is not the canonical operation surface.
