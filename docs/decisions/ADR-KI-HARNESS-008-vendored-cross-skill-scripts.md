...
- **Engine-level, not per-skill.** These are cross-skill tools that range over the whole tree, so they are vendored by an engine-level rule rather than through the per-skill `vendors:` frontmatter grammar — which is a per-skill, per-mode `{audit, conform, educate, help}` contract and cannot express a whole-tree tool. They belong to the same engine-level class as the `aggregate.ts` runner and the four `bin/` wrappers, which are likewise engine artifacts and not per-skill declarations.
