...
The bootstrap engine (`skills/ki-bootstrap/scripts/bootstrap.ts`) vendors three cross-skill operational scripts into a target's `.ki-meta/bin/`, but only when the resolved skill set includes `ki-harness`.
...
- **Canonical home is `skills/ki-bootstrap/scripts/`.** The three scripts move there from the harness repo root `scripts/`. `ki-harness` owns the standard for the `package.json` keys; `ki-bootstrap` owns the machinery, and the engine already shells to `skill-help.ts` at vendor time to render the per-skill HELP snapshots, so co-locating them makes the engine self-contained.
