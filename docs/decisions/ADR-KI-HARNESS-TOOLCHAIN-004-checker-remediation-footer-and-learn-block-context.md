...
- Both checks are covered by run-based behavioural tests (`audit-memory.test.ts`, `audit-tokenomics.test.ts`) wired into `ki:verify`, per the `ki-engineering` §6 convention that skill `scripts/` are run-tested rather than unit-tested.
...
- `skills/ki-engineering/references/checker-contract.md` — the amended output contract.
- `skills/ki-housekeeping/references/audit-rubric.md` (IDX-6) and `skills/ki-tokenomics/references/audit-rubric.md` (TOOL-4).
