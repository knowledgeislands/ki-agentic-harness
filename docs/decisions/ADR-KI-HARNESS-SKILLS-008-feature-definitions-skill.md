...
- The skill sits in the general-governance cluster ([ADR-KI-HARNESS-SKILLS-006](ADR-KI-HARNESS-SKILLS-006-skill-taxonomy-and-implication-graph.md)) — declared opt-in per repo (like `ki-decision-records`/`ki-project-roadmap`), has `depends-on: []`, and is not artifact-detected in `ki-repo`'s coverage cascade.
