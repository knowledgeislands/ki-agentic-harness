...
- `ki-project-roadmap` and `ki-tokenomics` remain separate standards; `ki-handoffs` composes on them at runtime by being run in sequence (ADR-KI-HARNESS-SKILLS-003), while staying valid standalone (ADR-KI-HARNESS-SKILLS-004).
...
- The skill slots into the canonical dependency order (ADR-KI-HARNESS-SKILLS-003) after `ki-project-roadmap` / `ki-tokenomics`, which it composes on.
