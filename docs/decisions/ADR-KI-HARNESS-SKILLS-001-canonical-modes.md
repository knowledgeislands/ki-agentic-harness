...
- **EDUCATE** — bring the skill's governance into being in a target: scaffold a new conformant artifact, or bring an off-standard one onto the floor from scratch. Its mechanical half is a per-skill `scripts/educate.ts` (the EDUCATE counterpart to `audit-*.ts`, a thin delegator into the central chain engine) that runs from the remote source with no skill installed, validates the selected skill's frontmatter `ki-depends-on:` declarations, and satisfies the self-sufficiency contract — vendored scripts, per-skill and repo-wide `ki:*` keys — set out as its own decision later in the reading order. **EDUCATE is mandatory for every governance skill, even when thin**: a skill that scaffolds no standalone artifact still has an EDUCATE, whose job is to vendor the skill's declared mechanical unit (the frontmatter `ki-vendors:` declaration) into the target's `.ki-meta/` via the chain.
