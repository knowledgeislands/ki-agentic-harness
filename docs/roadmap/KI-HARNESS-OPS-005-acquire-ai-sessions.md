...
updated_at: 2026-09-16T22:10:27Z
---
...
- [ ] Define stable ChatGPT project selectors and receiver mappings using immutable project identity, a controlled `<Domain>: <Topic>` display-name convention, and retained prior names as migration aliases.
...
The visible ChatGPT projects provide the first routing inventory. Use the controlled display form `<Domain>: <Topic>` so related projects group visibly and routing intent remains legible. Names are still review evidence rather than identity: acquisition configuration must bind the immutable source project ID, record the current name, and retain any earlier name as an alias.
...
- **Personal:** prefix `Festivals`, `Tattoos`, `Travel Ideas`, `Relationships`, `Van Build`, `Fitness and Wellbeing`, `Costa Rica`, `Productivity`, `Spirituality`, `Home Improvement`, and `Side Hustle Ideas` with `Personal:`; all map initially to `kit-principal`.
