...
11. [~] Add isolated filesystem tests for first installation, repeat/update installation, runtime selection, unsafe or conflicting paths, hook hand-off, repository-only bootstrap, and development-link behaviour. Confirm the normal path works after the remote source is removed. The user-install, hook, and repository-bootstrap coverage is complete; move the development-link cases with their `ki-repo` implementation before closing this step.
