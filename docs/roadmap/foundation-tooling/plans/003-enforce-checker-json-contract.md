...
Remaining work is to add the source-harness collector across the full shipped fleet, make rubric code/type/citation and judgment-prompt coverage mechanically enforceable, and resolve each aggregate code through rubric metadata for code-first human output such as `CHK-004: Canonical checker reporter`.
