...
Adopt `.ki/` as the single repository-local KI root, aligned with the established repository namespace: `.ki/self/skill/` holds authored local governance and `.ki/meta/` holds removable generated bootstrap state. `kisle` remains the end-user command name, not the repository directory name.
...
1. Inventory every read, write, manifest entry, runtime projection, guide, fixture, and Git rule that refers to `.ki-self` or `.ki-meta`. Define the exact target tree, ownership, and deletion rule: `.ki/self/skill/` is committed authored content and `.ki/meta/` is generated content; no other `.ki/` child gains implicit ownership.
2. Design the one-way current-state migration. It must relocate a proven canonical `.ki-self/` source and rebase its runtime links, generate or safely reconcile `.ki/meta/`, preserve unproven, altered, or unsafe old paths for manual resolution, and remove old paths only after the new footprint is validated. Do not retain runtime fallbacks, dual discovery paths, or an alternate legacy configuration after migration.
...
- `.ki-self/`, `.ki-meta/`, and the new `.ki/` footprint — current-state migration in this harness.
...
1. A migrated repository has one `.ki/` root with a regular committed `.ki/self/skill/SKILL.md` and a regular manifest-owned `.ki/meta/` tree; no proven legacy `.ki-self/` or `.ki-meta/` path remains.
