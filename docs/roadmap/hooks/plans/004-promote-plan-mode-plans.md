---
id: '004'
title: Promote Plan Mode plans into thematic roadmaps
status: in-progress
roadmap: hooks/promote-plan-mode-plans-into-thematic-roadmaps
blocks: —
blocked-by: —
---
...
- `docs/roadmap/<theme>/plans/` frontmatter requires a qualified `roadmap:` locator — a plan with no canonical theme-roadmap `Blocking` or `Next` item is misfiled per the near-horizon principle (`skills/general-governance/ki-project-roadmap/references/plan-format.md`). A Plan Mode plan has no such linkage; picking one is a judgment call, not a mechanical copy.
...
- `docs/decisions/references/runtime-feature-coverage.md` explicitly flags that Claude Code's "Plan Mode" and this repo's "plan" artifact are "same word, entirely different mechanisms" — bridging them silently on every `ExitPlanMode` risks polluting the thematic plan collection with throwaway plans the user never meant to keep.
...
- `ki-plan`'s `new` subcommand already carries the plan-format judgment. Promotion v1 deliberately narrows the write surface to the physical `<git-root>/docs/roadmap/<theme>/plans`, allocates from the union of valid global-index and on-disk ids, and refuses drift, malformed ids, duplicates, symlinked path components, or an existing destination.
...
- Claude Code's scratch `plansDirectory` and the governed repository destination are separate boundaries. Promotion v1 supports only the hooks' default `$HOME/.claude/plans` scratch root and the repository's thematic `docs/roadmap/<theme>/plans/` destination; it does not honor a Claude Code scratch override or an alternative repository plan tree.
...
5. ✓ Replace the initial `ki-plan` prose with the reviewed promotion transaction in `SKILL.md` and `references/lifecycle.md`:
   ...
   - Require a clean read-only `ki:project-roadmap:audit` before choosing an id or destination. Stop on any FAIL or WARN and on index/on-disk drift, a malformed id, or a duplicate id; do not repair unrelated roadmap state during promotion.
   - Support only the physical `<git-root>/docs/roadmap/<theme>/plans` destination. Validate the git root, `docs`, `roadmap`, theme, `plans`, and destination components in order: each existing component must be a real non-symlink path physically contained by the git root; theme and filename components must match their safe grammars and contain no separator; create a missing `plans` directory only after all read-only gates pass.
...
- **Successful promotion:** from a valid current-session v1 record and clean thematic profile, confirm theme and qualified-locator judgment, lossless section mapping, independent dependency fields (`blocks: —`, `blocked-by: —`), an exclusively created destination, correct global index/graph update, untouched scratch/state, and a clean post-promotion roadmap audit.
- **Regression and contract:** run `bun hooks/plan-stamp.test.ts`, `bun hooks/plan-sync.test.ts`, `bun run ki:project-roadmap:audit`, `bun run ki:skills:audit`, and `bun run ki:authoring:audit`; confirm `/ki-plan` help/catalogue/runtime claims use the same lifecycle order and that existing `new`, `execute`, `done`, and `status` behavior is unchanged outside the thematic repository destination.
...
A live check found that `${CLAUDE_SESSION_ID}` substitution in `SKILL.md` and the `plan-stamp.sh` state serialization both work, but the runtime does **not** fire the `PostToolUse(ExitPlanMode)` hook on the Claude Code SDK / editor-extension surface — only the interactive CLI does. So Step 7's "live authenticated smoke test" cannot be met on that surface; it was never an auth problem. Rather than make the bridge foundational, the decision is to keep `ki-plan` **repo-first**: plans are authored via `new` straight into `docs/roadmap/<theme>/plans/`, native Plan Mode's `~/.claude/plans/` scratch file is never canonical, and `promote` is an optional Claude-Code-CLI-only convenience. This removes any mandatory hook dependency and is portable to Codex (which has no Plan Mode). The plan remains `in-progress`; its substantive rescoping/closing is deliberately separate from the project-roadmap migration.
