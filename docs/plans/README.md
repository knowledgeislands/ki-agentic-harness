...
The four-horizon model lives in [ROADMAP.md](../../ROADMAP.md), not here — a plan is written when an item enters `Blocking` or `Next` and removed when it lands. The format is defined in the `ki-plans` skill's [plan-format.md](../../skills/general-governance/ki-plans/references/plan-format.md).
