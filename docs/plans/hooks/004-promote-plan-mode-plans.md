...
- `hooks/plan-stamp.sh` (`PostToolUse(ExitPlanMode)`) stamps YAML frontmatter (`status`, `created`, `cwd`) onto the Plan Mode scratch file in place and writes `~/.claude/plans/.state/<session_id>` as a plaintext scratch-path pointer. `hooks/plan-sync.sh` (`PostToolUse(TodoWrite)`) consumes that pointer to keep progress synchronized.
- Both hooks resolve and contain the scratch target under `$HOME/.claude/plans`, but the state boundary is not yet sufficient for promotion: session ids use a denylist rather than a bounded allowlist; `.state` is not proved to be a real non-symlink directory; a rejected stamp can leave an older valid pointer live; the pointer carries no trusted hook-event working-directory provenance; and the display `cwd` is written as an unquoted YAML scalar.
- Secure v1 state will be JSON with exactly the version, session id, resolved scratch path, and physically resolved hook-event working directory needed by both consumers. `plan-sync.sh` will temporarily retain safe read compatibility with legacy plaintext pointers; `/ki-plan promote` will reject legacy state because it lacks repository provenance.
...
- An initial prose-only implementation exists in this worktree, but adversarial review found that it did not secure the state provenance, id allocation, destination components, or no-clobber boundary above. Those edits are not implementation completion; the steps below reset the work honestly around the reviewed design.
...
7. Run the focused hook tests, plan/skill/authoring audits, Markdown checks, and the manual success and rejection scenarios below. Review the final diff adversarially before handing it off; keep the plan `in-progress` for orchestrator verification and closure.
