...
describe('ki-skills AUDIT wrapper', () => {
  ...
  test('accepts a top-level educator that delegates help to its local vendored module', () => {
    const { base, dir } = fixture()
    writeFileSync(
      join(dir, 'scripts', 'educate.ts'),
      [
        "import { resolve } from 'node:path'",
        "import { runSkillEducator } from './vendored/ki-bootstrap/educator.ts'",
        '',
        "runSkillEducator({ skill: 'ki-fixture-audit', source: resolve(import.meta.dirname, '..') })",
        ''
      ].join('\n')
    )
    try {
      ...
      expect(
        parsed.records.some(
          (record) => (record as { code?: unknown }).code === 'SCRIPT-8' && (record as { level?: unknown }).level === 'FAIL'
        )
      ).toBe(false)
    } finally {
      rmSync(base, { recursive: true, force: true })
