...
test('ki-skills EDUCATE refreshes only its own local payloads', () => {
  ...
  expect(existsSync(join(target, '.ki-meta', 'educators', 'ki-skills', 'educate.ts'))).toBe(true)
  expect(existsSync(join(target, '.ki-meta', 'educators', 'ki-skills', 'skill', 'scripts', 'educate.ts'))).toBe(true)
