...
const remaining = parsed.arguments.filter((argument, index) => argument !== '--memory-dir' && index !== memoryAt + 1 && argument !== '--dry-run')
...
if (remaining.length > 1) { process.stderr.write('error: conform accepts at most one repository path\n'); process.exit(2) }
const repoRoot = resolve(remaining[0] ?? '.')
const targetMemory = resolve(memoryDir ?? resolveMemoryDir(repoRoot))
const result = runChecker({ mode: 'conform', concern: KI_HOUSEKEEPING_RUBRIC.concern, target: targetMemory, rubric: KI_HOUSEKEEPING_RUBRIC, subjects: [{ familyCodes: KI_HOUSEKEEPING_FAMILY_CODES, context: () => createHousekeepingContext({ repoRoot, memoryDir: targetMemory, dryRun }) }] })
