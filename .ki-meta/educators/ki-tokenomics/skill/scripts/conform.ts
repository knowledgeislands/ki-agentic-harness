...
if (argv.includes('-h') || argv.includes('--help')) {
  process.stdout.write('Usage: bun scripts/conform.ts [target] [--no-user] [--user <dir>] [--dry-run] [--reporter jsonl|terminal]\n')
  process.exit(0)
}
...
if ((userAt >= 0 && !userDir) || targets.length > 1) {
  process.stderr.write('error: --user requires a value and conform accepts at most one target\n')
  process.exit(2)
}
  ...
  subjects: [
    {
      ...
      context: () => createTokenomicsContext({ target, noUser, userDir, dryRun, mode: 'conform' })
    }
  ]
})
