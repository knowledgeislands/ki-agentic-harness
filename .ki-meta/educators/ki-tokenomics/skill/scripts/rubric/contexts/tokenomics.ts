...
export type TokenomicsRubricContext = {
  audit: ReadonlyMap<string, readonly LegacyFinding[]>
  conform: ReadonlyMap<string, readonly LegacyFinding[]>
}
...
const collect = (source: string): ReadonlyMap<string, readonly LegacyFinding[]> => {
  ...
  for (const line of source.split(/\r?\n/)) {
    try {
      ...
      /* terminal output is not part of the machine response */
    }
  }
  return findings
}
...
export const createTokenomicsContext = ({
  target,
  noUser,
  userDir,
  dryRun,
...
const asOutcomes = <Outcome extends AuditOutcome | ConformOutcome>(
  findings: readonly LegacyFinding[] | undefined,
  conform: boolean
): RubricOutcomes<Outcome> => {
  if (!findings?.length) return [{ status: 'PASS', message: 'No finding for this criterion.' }] as unknown as RubricOutcomes<Outcome>
  ...
  })) as unknown as RubricOutcomes<Outcome>
}

export const auditOutcomes = (context: TokenomicsRubricContext, code: string): RubricOutcomes<AuditOutcome> =>
  asOutcomes<AuditOutcome>(context.audit.get(code), false)
export const conformOutcomes = (context: TokenomicsRubricContext, code: string): RubricOutcomes<ConformOutcome> =>
  asOutcomes<ConformOutcome>(context.conform.get(code), true)
