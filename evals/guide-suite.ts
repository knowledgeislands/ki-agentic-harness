...
try {
  ...
  run(`bun "${join(HARNESS, 'skills/keystone/ki-bootstrap/scripts/lib/repo-bootstrap.ts')}" "${gf}"`, HARNESS, gfEnv)
  ...
  run(`bun "${join(HARNESS, 'skills/keystone/ki-bootstrap/scripts/lib/repo-bootstrap.ts')}" "${pf}"`, HARNESS, pfEnv)
