...
try {
  ...
  run(`bun "${join(HARNESS, 'skills/keystone/ki-bootstrap/scripts/bootstrap.ts')}" "${gf}"`, HARNESS, gfEnv)
  ...
  run(`bun "${join(HARNESS, 'skills/keystone/ki-bootstrap/scripts/bootstrap.ts')}" "${pf}"`, HARNESS, pfEnv)
