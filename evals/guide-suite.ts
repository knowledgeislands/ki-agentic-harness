...
try {
  ...
  check(existsSync(join(gf, 'scripts/ki/aggregate.ts')), 'vendored the aggregate runner')
  const repoChecker = join(gf, 'scripts/ki/ki-repo/audit-repo.ts')
  ...
  check(existsSync(join(gf, 'scripts/ki/ki-mcp/audit-mcp.ts')), 'declared [ki-mcp] pulled ki-mcp into the vendored set')
  ...
  check(existsSync(join(lg, 'scripts/ki/ki-repo/audit-repo.ts')), '--legacy vendored the checkers')
  check(existsSync(join(lg, 'scripts/ki/aggregate.ts')), '--legacy installed the aggregate')
